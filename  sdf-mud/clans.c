...
char NULL_FILE[256] = "../system/TEMP.file";
...
void save_clan( CLAN_DATA * clan )
{
   ...
   else
   {
      ...
      if( clan->members && clan->members[0] != '\0' )
         fprintf( fp, "NameMembers  %s~\n", clan->members );
      ...
      fprintf( fp, "#END\n" );
      fclose( fp );
     //    bug("Save_clan: Error writing clan file.", 0 );
   }
...
{
   char newstring[MIL];
   char finishedstring[MIL];
   char *endstr;
   int length;

   if( ( endstr = strstr( council->members, name ) ) == NULL )
      return FALSE;

   length = strlen( council->members );
   length -= strlen( endstr );

   memcpy( newstring, council->members, length );
   newstring[length] = '\0';

   if( ( strlen( endstr ) + strlen( name ) + 1 ) >= strlen( council->members ) )
      sprintf( finishedstring, "%s%s", newstring, endstr + ( strlen( name ) + 1 ) );
   else
      sprintf( finishedstring, "%s%s", newstring, endstr + strlen( name ) );

   DISPOSE( council->members );
   council->members = str_dup( finishedstring );
...
{
   char buf[MIL];
   sprintf( buf, "%s %s", council->members, name );
   DISPOSE( council->members );
   council->members = str_dup( buf );
   save_council( council );
}
...
bool rem_clan_member( CLAN_DATA * clan, char *name )
{
   ...
   if( ( endstr = strstr( clan->members, name ) ) == NULL )
      return FALSE;

   length = strlen( clan->members );
   length -= strlen( endstr );

   memcpy( newstring, clan->members, length );
   newstring[length] = '\0';

   if( ( strlen( endstr ) + strlen( name ) + 1 ) >= strlen( clan->members ) )
      sprintf( finishedstring, "%s%s", newstring, endstr + ( strlen( name ) + 1 ) );
   else
      sprintf( finishedstring, "%s%s", newstring, endstr + strlen( name ) );

   DISPOSE( clan->members );
   clan->members = str_dup( finishedstring );
   save_clan( clan );
   return TRUE;
}
...
{
   char buf[MIL];
   sprintf( buf, "%s %s", clan->members, name );
   DISPOSE( clan->members );
   clan->members = str_dup( buf );
   save_clan( clan );
}
...
void save_council( COUNCIL_DATA * council )
{
   ...
   else
   {
      ...
      if( council->members && council->members[0] != '\0' )
         fprintf( fp, "NameMembers  %s~\n", council->members );
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'M':
            ...
            KEY( "Members", clan->num_members, fread_number( fp ) );
         ...
         case 'N':
            ...
            KEY( "NameMembers", clan->members, fread_string_nohash( fp ) );
...
void fread_council( COUNCIL_DATA * council, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'M':
            KEY( "Members", council->num_members, fread_number( fp ) );
            KEY( "Meeting", council->meeting, fread_number( fp ) );
            break;

         case 'N':
            KEY( "Name", council->name, fread_string( fp ) );
            KEY( "NameMembers", council->members, fread_string_nohash( fp ) );
            break;
...
/*
   ...
   if( clan->mem_limit && clan->num_members >= clan->mem_limit )
   {
...
/* 
 * The following added for Surrender, but could be used
 * as a method for a player to outcast themselves from
 * their own clan if so wished. -Garinan
 */

void do_qoutcast( CHAR_DATA * ch, char *argument )
{
   CLAN_DATA *clan;
   char buf[MAX_STRING_LENGTH];
   ...
   {
      ch_printf( ch, "Your status within %s has saved you from being outcast.\r\n", ch->pcdata->clan->name );
      return;
   }
   ...
   if( clan->clan_type != CLAN_GUILD && clan->clan_type != CLAN_ORDER && clan->clan_type != CLAN_NOKILL )
   {
      ...
      for( sn = 0; sn < top_sn; sn++ )
         if( skill_table[sn]->guild == ch->pcdata->clan->class && skill_table[sn]->name != NULL )
         {
   ...
   if( clan->clan_type != CLAN_GUILD )
   {
      sprintf( buf, "%s has been outcasted from %s!", ch->name, clan->name );
      do_info( ch, buf );
   }
   rem_clan_member( clan, ch->name );
...
void do_outcast( CHAR_DATA * ch, char *argument )
{
   ...
   if( clan->clan_type != CLAN_GUILD )
   {
      sprintf( buf, "%s has been outcasted from %s!", victim->name, clan->name );
      do_info( ch, buf );
   }
   rem_clan_member( clan, victim->name );
...
void do_council_outcast( CHAR_DATA * ch, char *argument )
{
   ...
   if( victim->pcdata->council != council )
   {
      ...
      {
         --council->num_members;
         if( council->num_members < 0 )
            council->num_members = 0;
         ...
         do_info( ch, buf );
         rem_council_member( council, victim->name );
         save_char_obj( victim );
         save_council( council );
         return;
      }
...
void do_setclan( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( " nummembers board recall storage guard1 guard2\r\n", ch );
      send_to_char( " addmember remmember\r\n", ch );
   ...
   if( !str_cmp( arg2, "badge" ) )
   {
      ...
      {
         send_to_char("Badge must be 15 characters or less.\r\n", ch );
         return;
      }
      STRFREE( clan->badge );
   ...
   if( !str_cmp( arg2, "nummembers" ) )
   {
      clan->num_members = atoi( argument );
      send_to_char( "Done.\r\n", ch );
      save_clan( clan );
      return;
   }
   ...
   {
      add_clan_member( clan, argument );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "remmember" ) )
   {
      if( !rem_clan_member( clan, argument ) )
      {
...
void do_setcouncil( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "nummembers" ) )
   {
      council->num_members = atoi( argument );
      send_to_char( "Done.\r\n", ch );
      save_council( council );
      return;
   }
   if( !str_cmp( arg2, "addmember" ) )
   {
      add_council_member( council, argument );
      send_to_char( "Done.\r\n", ch );
      return;
   }
   if( !str_cmp( arg2, "remmember" ) )
   {
      if( !rem_council_member( council, argument ) )
      {
         send_to_char( "Name not found.\r\n", ch );
         return;
      }

//        rem_council_member( council, argument );
      return;
   }
...
void do_showclan( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf_color( ch, "&wMembers  : &W%-6d   &wMLimit : &W%-6d   &wAlign  : &W%-6d",
                    clan->num_members, clan->mem_limit, clan->alignment );
   if( clan->clan_type == CLAN_GUILD )
      ch_printf_color( ch, "&wclass  : &W%d &w(&W%s&w)", clan->class, npc_class[clan->class] );
   send_to_char( "\r\n", ch );
   ch_printf_color( ch, "&wMembers  : &W%s\r\n", clan->members );
...
void do_clans( CHAR_DATA * ch, char *argument )
{
   ...
   {
      ch_printf( ch, "# of Members:  %d\r\n", clan->num_members );
      ch_printf( ch, "Current members: %s\r\n", clan->members );
   }
   set_char_color( AT_BLOOD, ch );
   ch_printf( ch, "\r\nDescription:  %s\r\n", clan->description );
   return;
...
void do_councils( CHAR_DATA * ch, char *argument )
{
   ...
   if( council->head2 == NULL )
      ch_printf_color( ch, "&cHead:     &w%s\r\n&cMembers:  &w%d\r\n", council->head, council->num_members );
   else
      ch_printf_color( ch, "&cCo-Heads:     &w%s &cand &w%s\r\n&cMembers:  &w%d\r\n",
                       council->head, council->head2, council->num_members );
   ch_printf_color( ch, "&cMembers: &w%s\r\n", council->members );
...
void do_set( CHAR_DATA * ch, char *argument )
{
   ...
   {
      send_to_char( "Syntax: set <field> <choice|none>\r\n", ch );
      send_to_char( "\r\n", ch );
      send_to_char( "  Fields:  number1 number2 \r\n", ch );
      return;
   }
