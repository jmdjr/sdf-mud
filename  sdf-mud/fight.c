...
int arena_death( CHAR_DATA * ch, CHAR_DATA * victim );  /* Scion arenacode in arena.c */
...
void save_pkdata(  )
{
   ...
   if( ( fpout = fopen( PKILL_FILE, "w" ) ) == NULL )
   {
      bug( "Cannot open pkills.dat for writting", 0 );
      perror( PKILL_FILE );
      return;
   }
   for( illegal_pk = first_pk; illegal_pk; illegal_pk = illegal_pk->next )
   {
      if( !illegal_pk->name || illegal_pk->name[0] == '\0' )
      {
         bug( "Save_pkills: blank pkill" );
         continue;
      }
      fprintf( fpout, "#PKILLS\n" );
      fprintf( fpout, "Name       %s~\n", illegal_pk->name );
      fprintf( fpout, "Vict       %s~\n", illegal_pk->vict );
...
void load_pkdata(  )
{
   ...
   {
      bug( "Cannot open pkills.dat", 0 );
      exit( 0 );
   }
}

void fread_pkdata( FILE * fp )
{
   char buf[MAX_STRING_LENGTH];
   char *word;
   bool fMatch;
   IPK_DATA *illegal_pk;
   CREATE( illegal_pk, IPK_DATA, 1 );
   LINK( illegal_pk, first_pk, last_pk, next, prev );
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'D':
            KEY( "Date", illegal_pk->date, fread_string( fp ) );
            break;
         ...
         case 'E':
            if( !str_cmp( word, "End" ) )
            {
               if( !illegal_pk->name )
               {
                  bug( "Fread_pkdata: Name not found", 0 );
                  free_pkdata( illegal_pk );
                  return;
               }
               if( !illegal_pk->vict )
               {
                  bug( "Fread_pkdata: Vict not found", 0 );
                  free_pkdata( illegal_pk );
                  return;
               }
               return;
         ...
         case 'N':
            KEY( "Name", illegal_pk->name, fread_string( fp ) );
            break;

         case 'V':
            KEY( "Vict", illegal_pk->vict, fread_string( fp ) );
            break;
      }
      if( !fMatch )
      {
         sprintf( buf, "Fread_pkdata: no match: %s", word );
         bug( buf, 0 );
      }
   }
}
...
bool is_attack_supressed( CHAR_DATA * ch )
{
   ...
   if( !IS_NPC( ch ) && ch->pcdata->arena && sysdata.arena != 2 )
      return TRUE;

   return FALSE;
}

int get_hitroll( CHAR_DATA * ch, bool real )
{
   int hitroll;
...
int get_damroll( CHAR_DATA * ch, bool real )
{
   int damroll;
...
void violence_update( void )
{
   ...
   for( ch = last_char; ch; lst_ch = ch, ch = gch_prev )
   {
      ...
      if( gch_prev && gch_prev->next != ch )
      {
         ...
         ch->prev = NULL;
         gch_prev = NULL;
         interpret( ch, "shout Thoric says, 'Prepare for the worst!'" );
      }
      ...
      /*
       * Anti powerlevel tanking code --Cronel 
       */
      if( IS_NPC( ch ) && !IS_NPC( victim ) && check_anti_tanking( ch, victim ) )
         victim = who_fighting( ch );

      retcode = rNONE;

      if( !IS_NPC( ch ) )
         if( ch->pcdata->arena && ch->pcdata->arena->status == 2 && sysdata.arena != 2 )
         {
      ...
      /*   Andy December 7 2008 - Removing bleeding
       *
            ...
            if( 50 + ( ch->level / 4 ) > number_percent(  ) )
            {
               ...
               switch ( attacktype )
               {
                  ...
                  case DFND_DISPELMAGIC:
                     if( victim->fighting && victim->first_affect )
                     {
         ...
         if( IS_AWAKE( rch ) && !rch->fighting )
         {
            ...
            if( !IS_NPC( ch ) || IS_AFFECTED( ch, AFF_CHARM ) )
            {
               if( ( ( !IS_NPC( rch ) && rch->desc )
                     || IS_AFFECTED( rch, AFF_CHARM ) ) && is_same_group( ch, rch ) && !is_safe( rch, victim ) )
...
ch_ret multi_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_sixth_attack ) + ( dual_bonus * 3.5 ) ) / 5 );
   if( number_percent(  ) < schance )
   {
   ...
   schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_seventh_attack ) + ( dual_bonus * 4 ) ) / 6 );
   if( number_percent(  ) < schance )
   {
   ...
   schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_eighth_attack ) + ( dual_bonus * 4.5 ) ) / 7 );
   if( number_percent(  ) < schance )
   {
...
ch_ret one_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   //bool victim_damaged = FALSE; // Lets us check to see if we do any damage or not
   int cur_victim_hp = victim->hit;
...
//    thac0     = interpolate( ch->level, thac0_00, thac0_32 ) - GET_HITROLL(ch);
   thac0 = interpolate( ch->level, thac0_00, thac0_32 ) - get_hitroll( ch, FALSE );
   ...
   {
        //victim_damaged = FALSE; // We didnt damage the victim --sywin
      /*
       * Physical hit with no damage (hit natural AC) -Seven
       */
      if( diceroll < thac0 - victim_ac
          && diceroll >=
          thac0 - ( 10 + ( dex_app[get_curr_dex( ch )].defensive / 10 ) + ( race_table[victim->race]->ac_plus / 10 ) )
      ...
      {
         //victim_damaged = FALSE; // We didn't damage the victim --sywin
         /*
          * Miss. 
          */
         if( prof_gsn != -1 )
      ...
      {
         victim_damaged = TRUE; //We passed all the fail checks, so we must have hit --sywin
      }
      */
   /*
    * Hit.
   ...
   if( diceroll == 19 ) //&& victim_damaged) //if we roll 19 and we do damage, then its a crit --sywin
   {
        /*
	if( dam <= 0 ) //If the damage is 0 or less, we will still deal some damage on a critcal hit//Ment as a work around only//
	{
	    dam = 1;
	}
...
//    dam += GET_DAMROLL(ch);
   ...
   if( diceroll == 19  && victim->hit < cur_victim_hp)
   {
      act( AT_FIRE, "You critically hit $N!", ch, NULL, victim, TO_CHAR );
      act( AT_FIRE, "$n critically hits you!", ch, NULL, victim, TO_VICT );
      act( AT_FIRE, "$n critically hits $N!", ch, NULL, victim, TO_NOTVICT );
...
/*
   ...
   if( char_died( victim ) )
      return rVICT_DIED;
*/ //Andy. Trying to get rid of the vch in NULL room bug.
...
ch_ret projectile_hit( CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * wield, OBJ_DATA * projectile, short dist )
{
   ...
   thac0 = interpolate( ch->level, thac0_00, thac0_32 ) - get_hitroll( ch, FALSE ) + ( dist * 2 );
   victim_ac = UMAX( -19, ( int )( GET_AC( victim ) / 10 ) );
...
ch_ret damage( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( !victim )
   {
      sprintf( buf, "Damage: null victim! Player: %s", ch->name );
      bug( buf, 0 );
      return rVICT_DIED;
   }
   ...
   if( !victim || !victim->name || victim->name[0] == '\0' )
   {
      if( victim )
         stop_fighting( victim, TRUE );

      bug( "Bad victim in fight.c, line %d. Checking Character List", __LINE__ );
      if( ch->next )
      {
         if( ch->next->prev != ch )
         {
            ...
            ch->next->prev = ch;
            interpret( ch, "shout Scion says to run for your lives!!" );
         }
      }
      if( ch->prev )
      {
         if( ch->prev->next != ch )
         {
            ...
            ch->prev->next = ch;
            interpret( ch, "shout Scion says, 'PANIC!!'" );
         }
      }
      return rNONE;
   }
    ...
    */
   /*
    * Josh -- Doubled max damage 
    */
   if( dt == gsn_backstab )
      maxdam = ( ch->level2 == -1 ) ? ch->level * 160 : ( ch->level3 == -1 ) ? ch->level2 * 80 : ch->level3 * 80;
   else
      maxdam = ( ch->level2 == -1 ) ? ch->level * 80 : ( ch->level3 == -1 ) ? ch->level2 * 40 : ch->level3 * 40;
   if( dam > maxdam && !( ch->level >= 115 ) && !IS_NPC( ch ) )
   {
 ...
 */
   ...
   if( dam > 10 && dt != TYPE_UNDEFINED )
   {
      ...
      if( damobj )
      {
         if( !in_arena( victim ) && !IS_OBJ_STAT( damobj, ITEM_NOSCRAP ) ) /* Don't damage eq in 
                                                                            * arena */
       ...
       /* Andy December 7 2008, removing bleed
        *
   ...
   /* 
    * Make sure trainers dont die
    */
   if ( victim->spec_fun == spec_lookup( "spec_trainer" ) && victim->hit < 1 )
       victim->hit = 1;
       ...
       && is_wielding_poisoned( ch ) && !IS_SET( victim->immune, RIS_POISON ) && !saves_poison_death( ch->level, victim ) )
   {
      ...
      /*
       * Was ch, supposed to have been victim -- Kratas 
       */
...
*/
   ...
   if( victim->position == POS_DEAD )
   {
      ...
      if( !npcvict )
      {
         ...
         if( !IS_NPC( ch ) && !IS_NPC( victim ) && victim->bounty > 0 && ch != victim && ch->name != victim->name )
         {
            ch->bowed += victim->bounty;
            ch->bkills++;
            if( !IS_IMMORTAL( ch ) )
               adjust_hiscore( "bounty", ch, ch->bkills );
            sprintf( log_buf, "&YYou have claimed %d gold from the head of %s!\r\n", victim->bounty, victim->name );
            send_to_char( log_buf, ch );
            send_to_char( "&YYou may collect your earnings at any bounty officer.&R\r\n", ch );
            victim->bounty = 0;
            sprintf( log_buf, "%s has claimed the bounty from the head of %s!", ch->name, victim->name );
            do_info( NULL, log_buf );
         }
...
bool is_safe( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( IS_SET( victim->in_room->room_flags, ROOM_SAFE ) || IS_SET( victim->in_room->room_flags, ROOM_SAFE ) )
   {
...
//    if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PACIFIST) ) /* Gorog */
   ...
   if( ( get_age( ch ) < 1 || ch->level < 5 ) && !in_arena( ch ) )   /* Scion arenacode */
   {
   ...
   if( ( get_age( victim ) < 1 || victim->level < 5 ) && !in_arena( victim ) )   /* Scion arenacode */
   {
      set_char_color( AT_WHITE, ch );
...
*/
   ...
   if( get_timer( victim, TIMER_PKILLED ) > 0 )
   {
      ...
      send_to_char( "That character has died within the last 15 minutes.\r\n", ch );
      return TRUE;
   }
   ...
   {
      set_char_color( AT_GREEN, ch );
      send_to_char( "You have been killed within the last 15 minutes.\r\n", ch );
      return TRUE;
   }
...
void check_killer( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( IS_NPC( victim ) )
   {
      if( !IS_NPC( ch ) )
      {
          ...
          * * by Joe Fabiano [email]-rinthos@yahoo.com[/email]
          * * on 03-16-03.
          */
         if( victim->level < 1 )
...
void update_pos( CHAR_DATA * victim )
{
   ...
   if( !IS_NPC( victim ) && IS_SET( victim->pcdata->flags, PCFLAG_NOSTUN ) )
   {
      if( victim->hit <= 0 )
         victim->position = POS_DEAD;
   }
...
neutral when they die given the difficulting of changing align */
   ...
   victim->pcdata->condition[COND_DRUNK] = 0;   /* Altrag */
   if( IS_VAMPIRE( victim ) )
   ...
   for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
   {
      ...
      for( obj = ch->first_carrying; obj; obj = obj_next )
      {
         ...
         if( ( IS_OBJ_STAT( obj, ITEM_ANTI_EVIL ) && IS_EVIL( ch ) )
             ...
             || ( IS_OBJ_STAT( obj, ITEM_ANTI_NEUTRAL ) && IS_NEUTRAL( ch ) ) || obj->level > ch->level )
         {
...
bool check_illegal_pk( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( !IS_NPC( victim ) && !IS_NPC( ch ) )
   {
          ...
          && !in_arena( ch ) && ch != victim && !( IS_IMMORTAL( ch ) && IS_IMMORTAL( victim ) ) )
      {
         ...
         if( !IS_NPC( ch ) && !IS_NPC( victim ) )
         {
            CREATE( illegal_pkill, IPK_DATA, 1 );
            LINK( illegal_pkill, first_pk, last_pk, next, prev );
            illegal_pkill->vict = victim->name;
            illegal_pkill->name = ch->name;
            illegal_pkill->date = ctime( &current_time );
            save_pkdata(  );
         }
         if( victim->bounty > 0 && !( ch->level - victim->level ) > 10 )
            return FALSE;
         if( victim->bounty > 0 && !IS_SET( victim->pcdata->flags, PCFLAG_DEADLY ) )
            return FALSE;
         if( !IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) && ch->belig == 0 )
         {
            sprintf( log_buf, "^z&RFor commiting this act you are now eligible to have a bounty on your head.^x\r\n" );
            send_to_char( log_buf, ch );
            ch->belig = 1;
         }
...
void do_panic( CHAR_DATA * ch, char *argument )
{
   ...
   interpret( ch, "chat HELP ME... arrg arrg ack ack ... aaaayeeeeaaaahhh" );
...
void do_flee( CHAR_DATA * ch, char *argument )
{
   ...
   if ( wf->spec_fun == spec_lookup( "spec_trainer" ) )
   {
      affect_strip( ch, gsn_sneak );
      xREMOVE_BIT( ch->affected_by, AFF_SNEAK );
      if( ch->mount && ch->mount->fighting )
         stop_fighting( ch->mount, TRUE );
      act( AT_FLEE, "You stop practicing!", ch, NULL, NULL, TO_CHAR );
      stop_fighting( ch, TRUE );
      stop_hunting( wf );
      stop_hating( wf );
      return;
...
void do_oldslay( CHAR_DATA * ch, char *argument )
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   ...
   else if( !str_cmp( arg2, "pokemon" ) )
   {
      act( AT_YELLOW, "You say 'Pikachu, I choose you!' and you throw a pokeball.", ch, NULL, victim, TO_CHAR );
      act( AT_YELLOW, "As the pokeball opens a Pikachu appears.", ch, NULL, victim, TO_CHAR );
      act( AT_YELLOW, "You shout 'Pikachu, electric shock!' and Pikachu shocks $N to death.", ch, NULL, victim, TO_CHAR );
      act( AT_YELLOW, "$n says 'Pikachu, I choose you!' and throws a pokeball.", ch, NULL, victim, TO_VICT );
      act( AT_YELLOW, "As the pokeball opens a Pikachu appears.", ch, NULL, victim, TO_VICT );
      act( AT_YELLOW, "$n shouts 'Pikcahu, electric shock!' and Pikachu shocks you, your dead!", ch, NULL, victim, TO_VICT );
      act( AT_YELLOW, "$n says 'Pikachu, I choose you!' and throws a pokeball.", ch, NULL, victim, TO_NOTVICT );
      act( AT_YELLOW, "As the pokeball opens a Pikachu appears.", ch, NULL, victim, TO_NOTVICT );
      act( AT_YELLOW, "$n shouts 'Pikachu, electric shock!' and Pikachu shocks $N to death.", ch, NULL, victim, TO_NOTVICT );
   }

   else if( !str_cmp( arg2, "dbz" ) )
   {
      act( AT_CYAN, "You shout 'kame-a-ma-a' and release an energyball in $N's direction.", ch, NULL, victim, TO_CHAR );
      act( AT_CYAN, "You send $N to the other dimension.", ch, NULL, victim, TO_CHAR );
      act( AT_CYAN, "$n shouts 'kame-a-ma-a' and releases an energyball in your direction.", ch, NULL, victim, TO_VICT );
      act( AT_CYAN, "You scream in pain and everything fades to black.", ch, NULL, victim, TO_VICT );
      act( AT_CYAN, "$n shouts 'kame-a-ma-a' and releases an energyball in $N's direction.", ch, NULL, victim, TO_NOTVICT );
      act( AT_CYAN, "$n sends $N to the other dimension.", ch, NULL, victim, TO_NOTVICT );
   }
   ...
   {
      act( AT_YELLOW, "You gesture, and a slavering demon appears.  With a horrible grin, the", ch, NULL, victim, TO_CHAR );
      act( AT_YELLOW, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, NULL, victim,
           TO_CHAR );
      act( AT_YELLOW, "$n gestures, and a slavering demon appears.  The foul creature turns on", ch, NULL, victim, TO_VICT );
      act( AT_YELLOW, "you with a horrible grin.   You scream in panic before being eaten alive.", ch, NULL, victim,
           TO_VICT );
      act( AT_YELLOW, "$n gestures, and a slavering demon appears.  With a horrible grin, the", ch, NULL, victim,
           TO_NOTVICT );
      act( AT_YELLOW, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, NULL, victim,
           TO_NOTVICT );
   }
   ...
   {
      act( AT_YELLOW, "You slay $N in cold blood!", ch, NULL, victim, TO_CHAR );
      act( AT_YELLOW, "$n slays you in cold blood!", ch, NULL, victim, TO_VICT );
      act( AT_YELLOW, "$n slays $N in cold blood!", ch, NULL, victim, TO_NOTVICT );
   }
...
void do_surrender( CHAR_DATA * ch, char *argument )
{
   ...
   /*
    * Are you not fighting/berserking/aggresive? If so, you can't surrender 
    */
   if( !who_fighting( ch ) )
   {
      if( ch->position == POS_FIGHTING
   ...
   if( IS_AFFECTED( ch, AFF_BERSERK ) )
   {
      send_to_char( "Surrender while beserking?  Unheard of!\r\n", ch );
      return;
   }
   if( !IS_NPC( ch ) && ch->position < POS_FIGHTING )
   {
      send_to_char( "You can't surrender in an aggressive stance. \r\n", ch );
      return;
   }
   if( IS_NPC( ch ) && ch->position <= POS_SLEEPING )
      return;
   ...
   if( !IS_NPC( ch ) )
   {
      act( AT_GREEN, "$n stops his hostility and surrenders.", ch, NULL, NULL, TO_ROOM );
      act( AT_GREEN, "You stop your hostility and surrender.", ch, NULL, NULL, TO_CHAR );

      /*
       * Stop fighting, sit down, remove all held weapons, and throw them on the ground 
       */
      /*
       * Have to stop fighting RAW and NOT through stop_fighting(). 
       */
      free_fight( ch );
      update_pos( ch );
      if( ( obj = get_eq_char( ch, WEAR_WIELD ) ) != NULL )
      {
         while( ( obj = get_eq_char( ch, WEAR_WIELD ) ) != NULL )
         {
            /*
             * Sometimes when it removes, it doesn't tell you 
             */
            do_remove( ch, obj->name );
            /*
             * Sometimes, your objects don't drop. 
             */
            do_drop( ch, obj->name );
         }
      }
      else if( ( obj = get_eq_char( ch, WEAR_MISSILE_WIELD ) ) != NULL )
      {
         do_remove( ch, obj->name );
         do_drop( ch, obj->name );
      }
      ...
      if( ch->level < 100 )
      {
         los = ( exp_level( ch, ch->level + 1 ) - exp_level( ch, ch->level ) ) * 0.5;
         sprintf( buf, "You've lost %d experience due to your cowardliness!", los );
         act( AT_SAY, buf, ch, NULL, NULL, TO_CHAR );
         gain_exp( ch, 0 - los );


    /*
     * Your detiy punishment 
     *
     *   if( victim && ch->pcdata->deity )
     *   {
     *      int level_ratio = URANGE( 1, victim->level / ch->level, 50 );
     *
     *      if( victim && victim->race == ch->pcdata->deity->npcrace )
     *         adjust_favor( ch, 1, level_ratio );
     *      else if( victim && victim->race == ch->pcdata->deity->npcfoe )
     *         adjust_favor( ch, 16, level_ratio );
     *      else
     *         adjust_favor( ch, 0, level_ratio );
     *   }
     */
         /*
          * Add Coward flag and auto-outcast if in a clan -Garinan 
          */
         if( !( xIS_SET( ch->act, PLR_COWARD ) ) )
            xSET_BIT( ch->act, PLR_COWARD );

         if( ch->pcdata->clan && ch->pcdata->clan != NULL )
         {
            WIClan = TRUE;
            sprintf( cname, "%s", ch->pcdata->clan_name );
            do_qoutcast( ch, "" );
         }

         if( ( !ch->pcdata->clan || ( ch->pcdata->clan == NULL ) ) && WIClan )
            ch_printf( ch, "%s has abandoned you due to your cowardly actions.\r\n", cname );
      }

      /*
       * Determine if the people your fighting with care to acknowledge your surrender. 
       */
      for( victim = ch->in_room->first_person; victim; victim = victim->next_in_room )
      {
         ...
         {
            /*
             * Important to set the hatred and hunting off before checking to see if they continue the fight. 
             */
            stop_hating( victim );
            stop_hunting( victim );

            /*
             * If person fighting you is a PC, then they gain some EXP 
             */
            /*
             * Will devise a way to turn this off for multiplayer servers 
             */
            if( !IS_NPC( victim ) )
               gain_exp( victim, get_exp_worth( ch ) * 0.5 );

            /*
             * All NPCs that are evil and have 60% of their hitpoints OR anyone berserking or aggressive will 
             * continue to attack you. 
             */
            /*
             * Hope to have hated races have a say in the decision as well. 
             */
            if( ( IS_NPC( victim ) && IS_EVIL( victim ) && ( victim->hit / victim->max_hit ) > .4 ) ||
                IS_AFFECTED( victim, AFF_BERSERK ) || victim->position < POS_FIGHTING )
            {
               act( AT_DANGER, "$N ignores your pathetic truce and continues attacking!", ch, NULL, victim, TO_CHAR );
               act( AT_DANGER, "$N ignores $n's surrender and continues to attack!", ch, NULL, victim, TO_ROOM );
            ...
            {
               /*
                * Stops fighting as well and doesn't hate or hunt you.
                */
               free_fight( victim );
               update_pos( victim );
               act( AT_GREEN, "You lower your offensive.", victim, NULL, victim, TO_VICT );
               act( AT_GREEN, "$n lowers $s offensive.", victim, NULL, NULL, TO_ROOM );
               act( AT_SAY, "$N says to you 'Leave this place now and never return.'", ch, NULL, victim, TO_CHAR );
               act( AT_SAY, "$N says to $n 'Leave this place now and never return.'", ch, NULL, victim, TO_ROOM );
               /*
                * AutoBinding Code for later
                */
            }
         }
      }
   ...
   {
      /*
       * Determine the effects of NPCs surrendering 
       */
      /*
       * NPCs will sell you to the slave market 
       */
   }
   return;
}
