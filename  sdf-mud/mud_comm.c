...
char *mprog_type_to_name( int type )
{
   switch ( type )
   {
      ...
      case USE_PROG:
         return "use_prog";
      case BRIBE_SILVER_PROG:
         return "bribe_silver_prog";
      case BRIBE_COPPER_PROG:
...
void do_mpstat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "Gold: %ld    Silver: %ld   Copper: %ld\r\n", victim->gold, victim->silver, victim->copper );
...
void do_mpmarry( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( bride ) )
      bride->pcdata->wedto = STRALLOC( groom->name );
   if( !IS_NPC( groom ) )
      groom->pcdata->wedto = STRALLOC( bride->name );
}
...
void do_mpat( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' || argument[0] == '\0' )
   {
      sprintf( arg, "Mpat - Bad Argument - Mobile %d", ch->pIndexData->vnum );
      progbug( arg, ch );
      return;
   }
   ...
   if( ( location = find_location( ch, arg ) ) == NULL )
   {
      ...
      sprintf( arg, "Mpat - No such location: \'%s\' Mobile %d", argument, ch->pIndexData->vnum );
...
void do_mp_practice( CHAR_DATA * ch, char *argument )
{
   ...
   {
      sprintf( buf, "$n attempts to tutor you in %s, but it's beyond your comprehension.", sname );
 ...
 //adept = GET_ADEPT( victim, sn );
   adept = get_adept_remort( victim, sn );
   if( ( victim->pcdata->learned[sn] >= adept ) || ( victim->pcdata->learned[sn] >= max ) )
   {
      sprintf( buf, "$n shows some knowledge of %s, but yours is clearly superior.", sname );
      act( AT_TELL, buf, ch, NULL, victim, TO_VICT );
      return;
   }
...
void do_mpapply( CHAR_DATA * ch, char *argument )
{
   ...
   to_channel( log_buf, "Auth", LEVEL_IMMORTAL );
