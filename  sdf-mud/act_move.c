...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
   ...
   if( !IS_IMMORTAL( ch ) && IS_SET( to_room->area->flags, AFLAG_CLOSED ) && ch->in_room->area != to_room->area )
   {
      send_to_char( "I'm afraid that you cannot go there!\r\n", ch );
      return rNONE;
   }
...
/*
   if( !IS_NPC( ch )
       && ( ch->level < skill_table[gsn_bashdoor]->skill_level[ch->class]
            || ch->level < skill_table[gsn_bashdoor]->skill_level[ch->class2]
            || ch->level < skill_table[gsn_bashdoor]->skill_level[ch->class3] ) )
   {
      send_to_char( "You're not enough of a warrior to bash doors!\r\n", ch );
      return;
   }
What was this supposed to do? -Andy
*/
   one_argument( argument, arg );
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( !str_cmp( argument, "list" ) )
         {
            ...
            {
               sprintf( vnum, "(%d)", ch->pcdata->memorize[slot] );
               room = get_room_index( ch->pcdata->memorize[slot] );
               ch_printf_color( ch, "&C&w%d:%s%s\r\n",
                                slot, ( IS_IMMORTAL( ch ) && room ? vnum : "" ), ( room ? room->name : "None" ) );
            }
