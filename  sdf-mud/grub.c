 ...
 */
bool go_eval_and( GO_STRUCT * r, int op_num )
{
   enum
   { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
      OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
 ...
 */
bool go_eval_or( GO_STRUCT * r, int op_num )
{
   enum
   { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
      OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
              ...
              bool sor_dir, bool or_sw, bool np_sw, bool nm_sw, bool ng_sw, bool d2_sw )
{
   ...
   for( po = first_object; po; po = po->next )  /* Loop through all objects   */
   {
      ...
      for( pa = po->first_affect; pa; pa = pa->next )
         go_accum_aff( &r, pa->location, pa->modifier );
      res = or_sw ? go_eval_or( &r, op_num ) : go_eval_and( &r, op_num );
...
void gr_read( CHAR_DATA * ch, int op_num, bool or_sw, int dis_num )
{
   ...
   char gender[] = "NMF";  /* convert sex to text       */
   char gclass[] = "MCTWVDRAPN";  /* convert class to text     */
   char grace[][3] =  /* convert race to text      */
   ...
   char gcouncil[][4] = { "   ", "Bld", "Cod", "HiC", "Rep", "Res", "PR ", "QC ", "Cod", "AC ",
      "Sym"
   };
   ...
   while( !feof( fp ) ) /* read each input record    */
   {
      ...
      if( res )   /* record is a match         */
      {
         ...
         {
            ch_printf( ch,
                       "\r\n%-12s %-2s %1s %-2s %1s %3s %3s %5s %11s %11s %11s %-15s %-6s %s\r\n",
                       "Name", "Lv", "S", "R", "C", "Cln", "Cou", "Room", "Gold", "Silver", "Copper", "Site", "Last", "Pk" );
            title_sw = TRUE;
         }
         if( tot_match <= dis_num ) /* print record if applicable */
            ch_printf( ch,
                       "%-12s %2hd %c %2s %c %3s %3s %5hd %11ld %5ld %5ld %-15s %6lu %c\r\n",
                       r.name, r.level, gender[( unsigned char )r.sex],
                       grace[( unsigned char )r.race], gclass[( unsigned char )r.class],
                       gclan[( unsigned char )r.clan],
                       gcouncil[( unsigned char )r.council], r.room, r.gold, r.silver, r.copper, r.site, r.last, r.pkill );
      }
      fread( &r, sizeof( r ), 1, fp );
   }
   fclose( fp );
...
void diag_ins( OBJ_INDEX_DATA * p, int siz, OBJ_INDEX_DATA ** f )
{
...
#define   DIAG_MAX_SIZE  1000
   ...
   if( !str_cmp( arg1, "ww" ) )
   {
      ...
      if( IS_SET( vic->pcdata->flags, PCFLAG_WATCH ) )
         send_to_pager( "PCFLAG_WATCH is TRUE.\r\n", ch );
      else
         send_to_pager( "PCFLAG_WATCH is FALSE.\r\n", ch );
      return;
   }
...
#define ZERO_MAX   1500
   ...
   {
      MOB_INDEX_DATA *pm;
      int drace, dclass, dis_num, vnum1, vnum2, dis_cou = 0;
      ...
      for( cou = 0; cou < MAX_KEY_HASH; cou++ )
      {
         if( mob_index_hash[cou] )
            ...
            {
               if( pm->vnum >= vnum1 && pm->vnum <= vnum2 && pm->race == drace && pm->class == dclass && dis_cou++ < dis_num )
