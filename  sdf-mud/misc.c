...
void do_drink( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( obj->item_type )
   {
      ...
      case ITEM_BLOOD:
         ...
         {
            amount = ( 10 + ch->level ) - ch->pcdata->condition[COND_BLOODTHIRST];
...
void do_eat( CHAR_DATA * ch, char *argument )
{
   ...
   location = ch->in_room;
    if(( IS_IMMORTAL( ch ) ) && !str_cmp( argument, "portal"))
       {       send_to_char("This parses.\r\n", ch);
               for( obj_p = location->first_content; obj_p; obj_p = obj_p->next_content )
		  {	
			sprintf(buf, "%s - %d", obj_p->name, obj_p->item_type);
			if(obj_p->item_type == ITEM_PORTAL)
				remove_portal(obj_p);
		  }
	}
...
void do_tamp( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( spipe = get_obj_carry( ch, arg ) ) == NULL )
   {
      send_to_char( "You aren't carrying that.\r\n", ch );
      return;
   }
   if( spipe->item_type != ITEM_PIPE )
   {
      send_to_char( "You can't tamp that.\r\n", ch );
      return;
   }
   if( !IS_SET( spipe->value[3], PIPE_TAMPED ) )
   {
      act( AT_ACTION, "You gently tamp $p.", ch, spipe, NULL, TO_CHAR );
      act( AT_ACTION, "$n gently tamps $p.", ch, spipe, NULL, TO_ROOM );
      SET_BIT( spipe->value[3], PIPE_TAMPED );
      return;
   }
...
void do_smoke( CHAR_DATA * ch, char *argument )
{
   ...
   if( spipe->item_type != ITEM_PIPE )
   {
      act( AT_ACTION, "You try to smoke $p... but it doesn't seem to work.", ch, spipe, NULL, TO_CHAR );
      act( AT_ACTION, "$n tries to smoke $p... (I wonder what $e's been putting in $s pipe?)", ch, spipe, NULL, TO_ROOM );
      return;
   }
   if( !IS_SET( spipe->value[3], PIPE_LIT ) )
   {
      act( AT_ACTION, "You try to smoke $p, but it's not lit.", ch, spipe, NULL, TO_CHAR );
      act( AT_ACTION, "$n tries to smoke $p, but it's not lit.", ch, spipe, NULL, TO_ROOM );
      return;
   }
   if( spipe->value[1] > 0 )
   {
      if( !oprog_use_trigger( ch, spipe, NULL, NULL ) )
      {
         act( AT_ACTION, "You draw thoughtfully from $p.", ch, spipe, NULL, TO_CHAR );
         act( AT_ACTION, "$n draws thoughtfully from $p.", ch, spipe, NULL, TO_ROOM );
      }

      if( IS_VALID_HERB( spipe->value[2] ) && spipe->value[2] < top_herb )
      {
         int sn = spipe->value[2] + TYPE_HERB;
         SKILLTYPE *skill = get_skilltype( sn );

         WAIT_STATE( ch, skill->beats );
         if( skill->spell_fun )
            obj_cast_spell( sn, UMIN( skill->min_level, ch->level ), ch, ch, NULL );
         if( obj_extracted( spipe ) )
            return;
      }
      else
         bug( "do_smoke: bad herb type %d", spipe->value[2] );

      SET_BIT( spipe->value[3], PIPE_HOT );
      if( --spipe->value[1] < 1 )
      {
         REMOVE_BIT( spipe->value[3], PIPE_LIT );
         SET_BIT( spipe->value[3], PIPE_DIRTY );
         SET_BIT( spipe->value[3], PIPE_FULLOFASH );
      }
   }
}
...
void do_light( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_SET( spipe->value[3], PIPE_LIT ) )
   {
      if( spipe->value[1] < 1 )
      {
         act( AT_ACTION, "You try to light $p, but it's empty.", ch, spipe, NULL, TO_CHAR );
         act( AT_ACTION, "$n tries to light $p, but it's empty.", ch, spipe, NULL, TO_ROOM );
         return;
      }
      act( AT_ACTION, "You carefully light $p.", ch, spipe, NULL, TO_CHAR );
      act( AT_ACTION, "$n carefully lights $p.", ch, spipe, NULL, TO_ROOM );
      SET_BIT( spipe->value[3], PIPE_LIT );
      return;
   }
 ...
 */
int get_value( long int gval, long int sval, long int cval )
{
 ...
 */
void conv_currency( CHAR_DATA * ch, long int tmpvalue )
{
