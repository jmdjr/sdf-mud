...
void say_spell( CHAR_DATA * ch, int sn )
{
   ...
   {
      if( rch != ch )
         act( AT_MAGIC, is_class( ch, rch->class ) ? buf : buf2, ch, NULL, rch, TO_VICT );
   }
 ...
 *
 */
int rd_parse( CHAR_DATA * ch, int level, char *n_exp )
{
    ...
    */
   if( !n_exp || !strlen( n_exp ) )
      return 0;

   /*
    * get rid of brackets if they surround the entire expresion
    */
   if( ( *n_exp == '(' ) && n_exp[strlen( n_exp ) - 1] == ')' )
   {
      n_exp[strlen( n_exp ) - 1] = '\0';
      n_exp++;
   }
   ...
   for( x = 0; x < len; ++x )
      if( !isdigit( n_exp[x] ) && !isspace( n_exp[x] ) )
         break;
   if( x == len )
      return atoi( n_exp );
...
int dice_parse( CHAR_DATA * ch, int level, char *exp2 )
{
            ...
            if( ( sn = find_spell( ch, arg1, TRUE ) ) < 0
                || ( !IS_NPC( ch )
                     && ( ch->level < skill_table[sn]->skill_level[ch->class]
                          && ch->level < skill_table[sn]->skill_level[ch->class2]
                          && ch->level < skill_table[sn]->skill_level[ch->class3] ) ) )
/*            || ( !IS_NPC(ch) && ch->level < SKILL_CLASS(ch, sn) ) ) */
            {
     ...
     //  if( !IS_NPC( ch ) && GET_ADEPT( ch, skill_lookup( skill->name ) ) )
         if( !IS_NPC( ch ) && get_adept_remort( ch, skill_lookup( skill->name ) ) )
            mana = mana - ( mana / 4 );
...
//       if( !IS_NPC( ch ) && GET_ADEPT( ch, skill_lookup( skill->name ) ) )
   ...
   else
   {
      ...
      else
      {
         ...
         schance = ( IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_second_cast ) + ( ch->level / 10 ) ) / 1.5 ) );
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_third_cast ) + ( ch->level / 10 * 1.5 ) ) / 2 );
         if( number_percent(  ) < schance )
         {
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fourth_cast ) + ( ch->level / 10 * 2 ) ) / 2.5 );
         if( number_percent(  ) < schance )
         {
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fifth_cast ) + ( ch->level / 10 * 2.5 ) ) / 3 );
         if( number_percent(  ) < schance )
         {
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_sixth_cast ) + ( ch->level / 10 * 3 ) ) / 3.5 );
         if( number_percent(  ) < schance )
         {
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_seventh_cast ) + ( ch->level / 10 * 3.5 ) ) / 4 );
         if( number_percent(  ) < schance )
         {
         ...
         schance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_eighth_cast ) + ( ch->level / 10 * 4 ) ) / 4.5 );
         if( number_percent(  ) < schance )
         {
...
/*    if ( !IS_NPC( victim ) && !IS_NPC( ch ) )
  ...
  -- What's wrong with a little charming between friends? -- Scion */
   ...
   if( IS_AFFECTED( victim, AFF_CHARM )
       ...
       || level < victim->level || circle_follow( victim, ch ) || saves_spell_staff( schance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
/* NEW Enchant Weapon - Badastaz, 2003 */
ch_ret spell_enchant_weapon( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   /*
    * Must be a Weapon, and NON-MAGIC .. Except for IMMORTALS - Badastaz 
    */
   if( ( obj->item_type != ITEM_WEAPON || IS_OBJ_STAT( obj, ITEM_MAGIC ) ) && !IS_IMMORTAL( ch ) )
   {
   ...
   /*
    * Generate BASE enchant chance 
    */
   aenchant = ( number_percent(  ) / 5 ) + ( ( get_curr_int( ch ) / 6 ) + ( get_curr_lck( ch ) / 10 ) );
   /*
    * REMOVE the object to save BUG/CRASH (In Case) 
    */
   separate_obj( obj );
   /*
    * Use ANY modifers to HR and DR to increase change of EXPLODE or FADE 
    */
   exchance = 0;
   ...
   {
      aenchant += exchance;   /* Increase EXPLODE chance (70%) */
      /*
       * LEVEL DIFF between OBJ and CH increases this 
       */
   ...
   {
      aenchant -= exchance;   /* Increase FADE chance (30%) */
      /*
       * LEVEL DIFF between OBJ and CH increases this 
       */
      if( obj->level > ch->level )
   ...
   /*
    * If Object becomes IMM level .. and caster NOT imm, do nothing 
    */
   /*
    * OR if object level exceeds MAX imm level - do nothing 
    */
   if( ( obj->level >= LEVEL_IMMORTAL && !IS_IMMORTAL( ch ) ) || obj->level >= LEVEL_SUPREME )
      aenchant = 6;
   ...
   else
   {
      ...
      if( IS_OBJ_STAT( obj, ITEM_MAGIC ) )   /* Immortals REMOVE the MAGIC flag on fade */
...
/* END NEW enchant_weapon - Badastaz */
...
/*	magic_bit_name( obj->magic_flags ), -- unused for now */
      ...
      if( obj->gold_cost > 0 && obj->silver_cost > 0 && obj->copper_cost > 0 )
         ch_printf( ch, "%ld Gold, %ld Silver, and %ld Copper\r\n", obj->gold_cost, obj->silver_cost, obj->copper_cost );
      else if( obj->gold_cost <= 0 && obj->silver_cost > 0 && obj->copper_cost > 0 )
         ch_printf( ch, "%ld Silver and %ld Copper\r\n", obj->silver_cost, obj->copper_cost );
      else if( obj->gold_cost <= 0 && obj->silver_cost <= 0 && obj->copper_cost > 0 )
         ch_printf( ch, "%ld Copper\r\n", obj->copper_cost );
      else if( obj->gold_cost <= 0 && obj->silver_cost > 0 && obj->copper_cost <= 0 )
         ch_printf( ch, "%ld Silver\r\n", obj->silver_cost );
      else if( obj->gold_cost > 0 && obj->silver_cost <= 0 && obj->copper_cost <= 0 )
         ch_printf( ch, "%ld Gold\r\n", obj->gold_cost );
      else if( obj->gold_cost > 0 && obj->silver_cost <= 0 && obj->copper_cost > 0 )
         ch_printf( ch, "%ld Gold and %ld Copper\r\n", obj->gold_cost, obj->copper_cost );
      else if( obj->gold_cost > 0 && obj->silver_cost > 0 && obj->copper_cost <= 0 )
         ch_printf( ch, "%ld Gold, and %ld Silver\r\n", obj->gold_cost, obj->silver_cost );
      else
         ch_printf( ch, "0 coins\r\n" );
...
/* Begin Weapon Identification Snippet  by Joe Fabiano */
            if( obj->value[3] == 0 )
               ch_printf( ch, "Weapon Type: Pugilism\r\n" );
            else if( obj->value[3] == 1 )
               ch_printf( ch, "Weapon Type: Long Blade\r\n" );
            else if( obj->value[3] == 2 )
               ch_printf( ch, "Weapon Type: Short Blade\r\n" );
            else if( obj->value[3] == 3 )
               ch_printf( ch, "Weapon Type: Sword\r\n" );
            else if( obj->value[3] == 4 )
               ch_printf( ch, "Weapon Type: Flexible Arm\r\n" );
            else if( obj->value[3] == 5 )
               ch_printf( ch, "Weapon Type: Talonous Arm\r\n" );
            else if( obj->value[3] == 6 )
               ch_printf( ch, "Weapon Type: Pugilism\r\n" );
            else if( obj->value[3] == 7 )
               ch_printf( ch, "Weapon Type: Bludgeon\r\n" );
            else if( obj->value[3] == 8 )
               ch_printf( ch, "Weapon Type: Bludgeon\r\n" );
            else if( obj->value[3] == 9 )
               ch_printf( ch, "Weapon Type: Unknown\r\n" ); /* What type would a v3 of 9 make it? Not sure-Joe */
               ...
               ch_printf( ch, "Weapon Type: Missile Weapon\r\n" );
/* that covers all of them -Joe */
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( !IS_NPC( victim ) )
   {
      ...
      to_channel( log_buf, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level ) );
   }
...
ch_ret spell_teleport_superior( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   {
      send_to_char( "You cannot find your destination.\r\n", ch );
      return rSPELL_FAILED;
   }
   ...
   if( skill->hit_vict && skill->hit_vict[0] != '\0' )
      act( AT_MAGIC, skill->hit_vict, ch, NULL, ch, TO_VICT );
   else
      act( AT_MAGIC, "You have been teleported by $n!", ch, NULL, ch, TO_VICT );
   if( skill->hit_room && skill->hit_room[0] != '\0' )
      act( AT_MAGIC, skill->hit_room, ch, NULL, ch, TO_ROOM );
   else
      act( AT_MAGIC, "$n slowly fades from view.", ch, NULL, ch, TO_ROOM );

   char_from_room( ch );
   char_to_room( ch, pRoomIndex );

   sprintf( buf, "%s teleported %s to room %d.", ch->name, ch->name, ch->in_room->vnum );
   log_string_plus( buf, LOG_NORMAL, ch->level );
   to_channel( buf, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level ) );

   if( skill->hit_dest && skill->hit_dest[0] != '\0' )
      act( AT_MAGIC, skill->hit_dest, ch, NULL, ch, TO_ROOM );
   else
      act( AT_MAGIC, "$n slowly fades into view.", ch, NULL, ch, TO_ROOM );
...
//     || IS_SET( ch->in_room->room_flags, ROOM_NO_RECALL )
       || IS_SET( ch->in_room->room_flags, ROOM_NO_ASTRAL )
// This is what to noastral flag is for, and there are many rooms that are prototype that
// shouldn't be. -Andy
   ...
   if( !skill->hit_char || skill->hit_char[0] == '\0' )
   {
      ...
      set_char_color( AT_MAGIC, victim);
      send_to_char( "With a piercing sound, a shimmering portal appears in front of you!\r\n", victim); 
   }
   else
      act( AT_MAGIC, skill->hit_char, ch, NULL, victim, TO_CHAR );
   if( !skill->hit_room || skill->hit_room[0] == '\0' )
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( ( pMobIndex = get_mob_index( ( short )abs( corpse->gold_cost ) ) ) == NULL )
   {
         ...
         switch ( SPELL_POWER( skill ) )  /* clone object */
         {
            ...
            case SP_MINOR:
               if( ch->level - obj->level < 20
                   || get_value( obj->gold_cost, obj->silver_cost, obj->copper_cost ) > ch->level * get_curr_int( ch ) / 5 )
               {
                  failed_casting( skill, ch, NULL, obj );
                  return rNONE;
               }
...
ch_ret spell_life_transfer( int sn, int level, CHAR_DATA * ch, void *vo )
{
   CHAR_DATA *victim;
   CHAR_DATA *extra;
   int amt;
   ...
   extra = who_fighting( victim );

   if( extra != NULL && !IS_NPC( extra ) )
   {
      send_to_char( "&BYou failed.\r\n", ch );
      return rSPELL_FAILED;
   }

   if( ( victim->hit + ( ch->hit / 2 ) ) > victim->max_hit )
   {
      amt = ( victim->max_hit - victim->hit );
      victim->hit += amt;
      ch->hit -= amt;
   }
   else
   ...
   act( AT_BLUE, "You give half your life to $N.", ch, NULL, victim, TO_CHAR );
   act( AT_BLUE, "$n gives you half of their life.", ch, NULL, victim, TO_VICT );
   act( AT_BLUE, "$n gives half of their life to $N.", ch, NULL, victim, TO_NOTVICT );

   return rNONE;
}

ch_ret spell_custos_angelus( int sn, int level, CHAR_DATA * ch, void *vo )
{
   MOB_INDEX_DATA *pMobIndex;
   CHAR_DATA *victim;
   char buf[MAX_INPUT_LENGTH];
   
   pMobIndex = get_mob_index( 1203 );
   victim = create_mobile( pMobIndex );
   char_to_room( victim, ch->in_room );
   act( AT_MAGIC, "$n summons a guardian angel to protect $m.", ch, NULL, NULL, TO_ROOM );
   act( AT_MAGIC, "You summon a guardian anagel to protect you.", ch, NULL, NULL, TO_CHAR);
   sprintf( buf, "%s's Guardian Angel", ch->name );
   add_follower( victim, ch );
   xSET_BIT( victim->affected_by, AFF_CHARM );
   victim->short_descr = STRALLOC(buf);
   victim->hitroll = ch->level;
   victim->damroll = ch->level;
   victim->armor = (ch->level * -1) + -100;
   victim->level = ch->level;
   victim->hit = (ch->level * 5) + 500;
...
ch_ret spell_enchant_armor( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   /*
    * Must be ARMOR, and NON-MAGIC .. Except for IMMORTALS - {Zha} 
    */
   if( ( obj->item_type != ITEM_ARMOR || IS_OBJ_STAT( obj, ITEM_MAGIC ) ) && !IS_IMMORTAL( ch ) )
   {
      act( AT_MAGIC, "Winding and twisting about $p your magic fails to take hold.", ch, obj, NULL, TO_CHAR );
      act( AT_MAGIC, "As energy twists and winds about $p, $n's magic fails to take hold.", ch, obj, NULL, TO_NOTVICT );
      return rSPELL_FAILED;
