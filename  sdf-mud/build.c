...
char *const r_flags[] = {
   ...
   "nosupplicate", "arena", "nomissile", "amplify", "locker", "prototype",
   "wilderness"
};
...
char *const o_flags[] = {
   ...
   "nolocate", "groundrot", "artifact", "plrbld", "noscrap"
};
...
char *const area_flags[] = {
   "nopkill", "freekill", "noteleport", "resetonboot", "ploaded", "exitnames",
   "closed", "prototype", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15",
   "r16", "r17", "r18", "r19", "r20", "r21", "r22", "r23", "r24", "r25", "r26",
   "r27", "r28", "r29", "r30", "r31"
};
...
char *const o_types[] = {
   ...
   "shovel", "salve", "cook", "keyring", "piece", "odor", "silver", "copper"
};
...
char *const act_flags[] = {
   "npc", "sentinel", "scavenger", "undertaker", "r2", "aggressive", "stayarea",
   "wimpy", "pet", "train", "practice", "immortal", "deadly", "polyself",
   "meta_aggr", "guardian", "running", "nowander", "mountable", "mounted",
   "scholar", "secretive", "healer", "mobinvis", "noassist", "autonomous",
   "pacifist", "noattack", "annoying", "r13", "prototype", "banker", "noquest",
   "bounty", "gloryshop", "r14"
};

char *const pc_flags[] = {
   "r1", "deadly", "unauthed", "norecall", "nointro", "gag", "retired", "guest",
   "nosummon", "pager", "notitled", "groupwho", "diagnose", "highgag", "watch",
   "nstart", "immproof", "nobeep", "anonymous", "inductor", "married", "r15",
   "r16", "r17", "r18", "r19", "r20", "notell", "r22", "r23", "r24", "r25"
};
...
char *const plr_flags[] = {
   ...
   "needhelp", "questor", "newmoney", "coward", "exempt"
};
...
char *const mprog_flags[] = {
   ...
   "bribe_copper", "bribe_silver"
};

#ifdef COMPRESSION_TEST
void decompressRoom args( ( ROOM_INDEX_DATA * room ) );
bool compressRoom args( ( ROOM_INDEX_DATA * room ) );
#endif
...
void stop_editing( CHAR_DATA * ch )
{
   ...
   DISPOSE( ch->editor );
   ch->editor = NULL;
   act( AT_ACTION, "$n stops writing.", ch, NULL, NULL, TO_CANSEE );
   send_to_char( "Done.\r\n", ch );
   ch->dest_buf = NULL;
   ch->spare_ptr = NULL;
...
void do_goto( CHAR_DATA * ch, char *argument )
{
   ...
   for( fch = in_room->first_person; fch; fch = fch_next )
   {
      ...
      if( fch != ch
          && ( ( get_trust( fch ) >= ( IS_NPC( ch ) ? ch->level : ch->pcdata->wizinvis ) )
               || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
      {
         sprintf( buf, "&Y%s&w\r\n", ( ch->pcdata && ch->pcdata->bamfout
                                       && ch->pcdata->bamfout[0] != '\0' ) ? ch->pcdata->bamfout : buf2 );
   ...
   for( fch = ch->in_room->first_person; fch; fch = fch_next )
   {
               ...
               || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
      {
         sprintf( buf, "&Y%s&w\r\n", ( ch->pcdata && ch->pcdata->bamfin
                                       && ch->pcdata->bamfin[0] != '\0' ) ? ch->pcdata->bamfin : buf2 );
         send_to_char( buf, fch );
      }
   }
...
void do_mset( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' || ( arg2[0] == '\0' && ch->substate != SUB_REPEATCMD ) || !str_cmp( arg1, "?" ) )
   {
      ...
      send_to_char( "  hp mana move practice align race class2\r\n", ch );
      send_to_char( "  class3 hitroll damroll armor affected level\r\n", ch );
   ...
   if( !str_cmp( arg2, "Balance" ) )
   {
      if( IS_NPC( victim ) )
      {
         send_to_char( "You can't modify an npc's balance!\r\n", ch );
         return;
      }
      if( value < 0 )
      {
         send_to_char( "Value must be at or above 0!\r\n", ch );
         return;
      }
   ...
   if( !str_cmp( arg2, "level2" ) )
   {
      ...
      if( value < -1 || value > LEVEL_AVATAR + 5 )
      {
         ch_printf( ch, "Level2 range is -1 to %d.\r\n", LEVEL_AVATAR + 5 );
         return;
      }
      victim->level2 = value;
      return;
   }
   if( !str_cmp( arg2, "level3" ) )
   {
      if( !can_mmodify( ch, victim ) )
         return;
      ...
      if( value < -1 || value > LEVEL_AVATAR + 5 )
      {
         ch_printf( ch, "Level3 range is -1 to %d.\r\n", LEVEL_AVATAR + 5 );
         return;
      }
      victim->level3 = value;
      return;
   ...
   if( !str_cmp( arg2, "clan" ) )
   {
      ...
      if( !arg3 || arg3[0] == '\0' )
      {
         ...
         if( !IS_IMMORTAL( victim ) )
         {
            ...
            if( victim->pcdata->clan->num_members < 0 )
               victim->pcdata->clan->num_members = 0;
   ...
   if( !str_cmp( arg2, "council" ) )
   {
      ...
      if( !arg3 || arg3[0] == '\0' )
      {
         ...
         {
            --victim->pcdata->council->num_members;
            if( victim->pcdata->council->num_members < 0 )
               victim->pcdata->council->num_members = 0;
   ...
   if( !str_cmp( arg2, "council2" ) )
   {
      ...
      if( !arg3 || arg3[0] == '\0' )
      {
         ...
         {
            --victim->pcdata->council2->num_members;
            if( victim->pcdata->council2->num_members < 0 )
               victim->pcdata->council2->num_members = 0;
...
/* Taking out scion-ware for the simpler origional stuff that works
...
*/
   ...
   if( !str_cmp( arg2, "speaks" ) )
   {
      ...
      if( !IS_NPC( victim ) )
      {
         ...
         if( !knows_language( victim, victim->speaking ) )
...
void do_redit( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( ch->substate )
   {
      ...
      case SUB_ROOM_DESC:
         ...
         location->description = copy_buffer_nohash( ch );
         stop_editing( ch );
#ifdef COMPRESSION_TEST
         compressRoom( location );
#endif
         /*
          * }
          * else
          * {
          * DISPOSE( location->compress );
          * STRFREE( location->description );
          * location->description = copy_buffer_nohash(ch);
          * stop_editing( ch );
          * compress = compressRoom( location );
          * } 
          */
         ch->substate = ch->tempnum;
         return;
...
#ifdef COMPRESSION_TEST
      if( !location->compress )
         start_editing( ch, location->description );
      else
      {
         decompressRoom( location );
         start_editing( ch, location->description );
      }
#else
      start_editing( ch, location->description );
   ...
   if( !str_cmp( arg, "sector" ) )
   {
      ...
      else
      {
         for( i = 0; i < SECT_MAX; i++ )
            if( !str_prefix( argument, ( char * )sector_translation[i][1] ) )
            {
               temp = i;
               break;
            }
      ...
      if( temp < 0 || temp >= SECT_MAX )
      {
         location->sector_type = 0;
         send_to_char( "Sector not found. Defaulting to Inside.\r\n", ch );
      }
      else
      {
         location->sector_type = temp;
         send_to_char( "Done.\r\n", ch );
      }
      return;
   }
...
void fold_area( AREA_DATA * tarea, char *filename, bool install )
{
   ...
   for( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
   {
      ...
      fprintf( fpout, "%d\n", pMobIndex->exp );
      fprintf( fpout, "%ld %ld %ld\n", pMobIndex->gold, pMobIndex->silver, pMobIndex->copper );
/* Need to convert to new positions correctly on loadup sigh -Shaddai */
   ...
   for( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ )
   {
      ...
      fprintf( fpout, "%d %ld %ld %ld %ld\n", pObjIndex->weight,
               pObjIndex->gold_cost,
               pObjIndex->silver_cost,
               pObjIndex->copper_cost, pObjIndex->rent ? pObjIndex->rent : ( long int )( pObjIndex->gold_cost / 10 ) );
...
void do_astat( CHAR_DATA * ch, char *argument )
{
   ...
   if( !proto )
   {
      ...
      ch_printf_color( ch, "&wGold Looted:  &W%ld\r\n", tarea->gold_looted );
      ch_printf_color( ch, "&wSilver Looted: &W%ld\r\n", tarea->silver_looted );
      ch_printf_color( ch, "&wCopper Looted: &W%ld\r\n", tarea->copper_looted );
...
bool check_for_area_conflicts( AREA_DATA * carea, int val, int rom)
{
   AREA_DATA *area;
   for( area = first_area; area; area = area->next )
   {
      if( area != carea && check_area_conflict( area, val ,rom) )
	{
	          return TRUE;
	}
   }
...
void do_aset( CHAR_DATA * ch, char *argument )
{
   ...
   {
      if( check_for_area_conflicts(  tarea, vnum ,1) )
      {
   ...
   {
      if( check_for_area_conflicts(tarea, vnum ,2) )
      {
         send_to_char( "That would conflict with another area.\r\n", ch );
         return;
      }
   ...
   {
      if( check_for_area_conflicts(  tarea, vnum ,3) )
      {
         send_to_char( "That would conflict with another area.\r\n", ch );
         return;
      }
...
void do_olist( CHAR_DATA * ch, char *argument )
{
   ...
   for( vnum = lrange; vnum <= trange; vnum++ )
   {
      ...
      if( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
         ...
         pro = "NP";
      pager_printf( ch, "&G%5d (%-2s) %-20s (%s)\r\n", vnum, pro, obj->name, obj->short_descr );
   }
   return;
}
