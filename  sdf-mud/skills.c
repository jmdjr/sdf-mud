...
/*
 *  Changed hanging 'if' statements to nested to avoid default TRUE 
 *  evaluation -Mordred January 2 2009
 */
...
void do_sset( CHAR_DATA * ch, char *argument )
{
   ...
   if( get_trust( ch ) > LEVEL_GREATER && !str_cmp( arg1, "copy" ) )
   {
      ...
      for( sindex = 0; sindex < top_sn; sindex++ )
      {
         skill_table[sindex]->skill_level[class2] =
            ( skill_table[sindex]->skill_level[class1] <= MAX_LEVEL ? skill_table[sindex]->skill_level[class1] : 101 );
         skill_table[sindex]->skill_adept[class2] =
            ( skill_table[sindex]->skill_adept[class1] <= MAX_LEVEL ? skill_table[sindex]->skill_adept[class1] : 101 );
      }
   ...
   if( fAll )
   {
      for( sn = 0; sn < top_sn; sn++ )
      {
         ...
         if( skill_table[sn]->name
              ...
              */
             && ( victim->level >= SKILL_CLASS( victim, sn ) || value == 0 ) )
         {
...
// adept = GET_ADEPT( ch, sn );
   adept = get_adept_remort( ch, sn );
   sklvl = skill_table[sn]->skill_level[ch->class];
   if( sklvl > 100 )    /* Check if not in skill list - may not handle remort -Mordred */
        return;
   if( sklvl == 0 )
        sklvl = ch->level;
...
{
   int adept, schance, sklvl;
   sklvl = skill_table[sn]->skill_level[ch->class];
   if( sklvl > 100 )    /* testing for not in class slist - may not handle remort --Mordred */
       return;
   if( IS_NPC( ch ) || ch->pcdata->learned[sn] <= 0 )
      return;
   schance = ch->pcdata->learned[sn] + ( 5 * skill_table[sn]->difficulty );
...
/*    if ( !IS_NPC( ch ) && !IS_NPC( victim ) )
 ...
 Pstealing is cool! -- Scion */
   ...
   if( victim->position == POS_FIGHTING || !can_use_skill( ch, percent, gsn_steal ) )
   {
      ...
      sprintf( buf, "yell %s is a bloody thief!", ch->name );
...
/* New Backstab - Orion Elder */
void do_backstab( CHAR_DATA * ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   OBJ_DATA *obj = NULL;
   OBJ_DATA *objtemp1 = NULL;
   OBJ_DATA *objtemp2 = NULL;
   bool obj1stab = FALSE;
   bool obj2stab = FALSE;
   int obj1dam = 0;
   int obj2dam = 0;
   ...
   objtemp1 = get_eq_char( ch, WEAR_WIELD );
   objtemp2 = get_eq_char( ch, WEAR_DUAL_WIELD );

   if( ( objtemp1 == NULL ) && ( objtemp2 == NULL ) )
   {
      send_to_char( "You need to be wielding a weapon to do that!\r\n", ch );
      return;
   }

   if( objtemp1 && ( objtemp1->value[3] == 11 || objtemp1->value[3] == 2 ) )
   {
      {
         obj1stab = TRUE;
      }
   }
   if( objtemp2 && ( objtemp2->value[3] == 11 || objtemp2->value[3] == 2 ) )
   {
      {
         obj2stab = TRUE;
      }
   ...
   if( !obj1stab )
   {
      if( !obj2stab )
      {
         send_to_char( "You must be wielding a piercing or stabbing weapon to do that!\r\n", ch );
         return;
      }
   }
   ...
   if( objtemp2 )
   {
      if( obj1stab && obj2stab )
      {
         obj1dam = ( objtemp1->value[1] + objtemp1->value[2] ) / 2;
         obj2dam = ( objtemp2->value[1] + objtemp2->value[2] ) / 2;
         if( obj1dam >= obj2dam )
            obj = objtemp1;
         else
            obj = objtemp2;
      }
      else if( obj1stab && !obj2stab )
      {
         obj = objtemp1;
      }
      else if( !obj1stab && obj2stab )
    ...
    */
   if( obj && obj->value[3] != 11 && obj->value[3] != 2 )
   {
...
void do_kick( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_kick ) )
   {
...
void do_punch( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_punch ) )
   {
      send_to_char( "You better leave the martial arts to fighters.\r\n", ch );
      return;
   }
...
void do_bite( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_bite ) )
   {
...
void do_claw( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_claw ) )
   {
      send_to_char( "That isn't quite one of your natural skills.\r\n", ch );
      return;
   }
...
void do_sting( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_sting ) )
   {
      send_to_char( "That isn't quite one of your natural skills.\r\n", ch );
      return;
   }
...
void do_tail( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_tail ) )
   {
      send_to_char( "That isn't quite one of your natural skills.\r\n", ch );
      return;
   }
...
void do_bash( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_bash ) )
   {
      send_to_char( "You better leave the martial arts to fighters.\r\n", ch );
      return;
   }
...
void do_stun( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_stun ) )
   {
      send_to_char( "You better leave the martial arts to fighters.\r\n", ch );
      return;
   }
...
void do_disarm( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_disarm ) )
   {
   ...
   if( can_use_skill( ch, ( percent * 3 / 2 ), gsn_disarm ) )
   {
      ...
      if( IS_OBJ_STAT( obj, ITEM_LOYAL ) )
      {
         ...
         act( AT_WHITE, "$n DISARMS you, but $p returns to your hand!", ch, obj, victim, TO_VICT );

         act( AT_WHITE, "You disarm $N, but $p returns to $s hand!", ch, obj, victim, TO_CHAR );

         act( AT_WHITE, "$n disarms $N, but $p returns to $s hand!", ch, obj, victim, TO_NOTVICT );
...
void do_meditate( CHAR_DATA * ch, char *argument )
{
   if( IS_NPC(ch) )
   {
      send_to_char("Mobs don't need meditate!", ch);
      return; /* Mobs  using meditate was crashing the MUD - Andy */
   }
   if( ch->mental_state >= 20 )
   {
        send_to_char("Your brain is going too fast to concentrate!\r\n", ch );
        return;
   }
   if ( ch->pcdata->condition[COND_FULL] <=10 && ch->pcdata->condition[COND_THIRST] <=10 )
   {
        send_to_char("You are too hungry AND thirst to concentrate. Take care of yourself!\r\n", ch);
        return;
   }
   if( ch->pcdata->condition[COND_FULL] <=10 )
   {
        send_to_char("You're too hungry to concentrate!\r\n", ch );
        return;
   }
   if( ch->pcdata->condition[COND_THIRST] <=10 )
   {
        send_to_char("You're too thirsty to concentrate!\r\n", ch );
        return;
   }
   if( !can_use_skill( ch, number_percent(  ), gsn_meditate ) )
   {
      send_to_char( "You spend several minutes in deep concentration, but fail to collect any mana.\r\n", ch );
      act(AT_ACTION, "$n spends several minutes in deep meditation.\r\n", ch, NULL, NULL, TO_ROOM);
      learn_from_failure( ch, gsn_meditate );
      WAIT_STATE( ch, skill_table[gsn_meditate]->beats );
      return;
   }
   send_to_char("You meditate peacefully, collecting mana from the cosmos.\r\n", ch);
   act( AT_ACTION, "$n meditates peacefully, collecting mana from the cosmos\r\n.", ch, NULL, NULL, TO_ROOM);
   learn_from_success(ch, gsn_meditate);
   WAIT_STATE( ch, skill_table[gsn_meditate]->beats );
   if( ch->max_mana > ch->mana + get_curr_int(ch) )
   {
      ch->mana = ch->mana + get_curr_int( ch );
   } else 
   {
      ch->mana = ch->max_mana;
   }
   if( ch->max_move > ch->move + (get_curr_dex(ch) * 15) )
   {
      ch->move = ch->move + (get_curr_dex(ch) * 15);
   } else
   {
      ch->move = ch->max_move;
   }
...
void do_trance( CHAR_DATA * ch, char *argument )
{
   ...
   {
      send_to_char( "You spend several minutes in a deep trance, but fail to collect any mana.\r\n", ch );
      act(AT_ACTION, "$n enters a peaceful trance for several minutes.\r\n", ch, NULL, NULL, TO_ROOM);
      learn_from_failure( ch, gsn_trance );
      WAIT_STATE( ch, skill_table[gsn_meditate]->beats );
      return;
   }
   send_to_char("You enter a peaceful trance, collecting mana from the cosmos.\r\n", ch);
   act( AT_ACTION, "$n enters a peaceful trance, collecting mana from the cosmos.", ch, NULL, NULL, TO_ROOM);
   learn_from_success(ch, gsn_trance);
   WAIT_STATE( ch, skill_table[gsn_trance]->beats );
   if( ch->max_mana > ch->mana + get_curr_int(ch) )
   {
      ch->mana = ch->mana + get_curr_int( ch ) + get_curr_wis( ch ) + 5;
   } else 
   {
      ch->mana = ch->max_mana;
   }
...
void do_ruminate( CHAR_DATA * ch, char *argument )
{
   if( ch->mental_state >= 20 )
   {
        send_to_char("Your brain is going too fast to concentrate!\r\n", ch );
   ...
   if( !can_use_skill( ch, number_percent(  ), gsn_ruminate ) )
   {
      send_to_char( "You ruminate deeply for a while but gain no mana.\r\n", ch );
      act(AT_ACTION, "$n ruminates deeply for a while.\r\n", ch, NULL, NULL, TO_ROOM);
      learn_from_failure( ch, gsn_ruminate );
      WAIT_STATE( ch, skill_table[gsn_meditate]->beats );
      return;
   }
   send_to_char("You slip into a deep rumination and channel mana from the void.\r\n", ch);
   act( AT_ACTION, "$n ruminates deeply for a while and channels mana from the void.\r\n", ch, NULL, NULL, TO_ROOM);
   learn_from_success(ch, gsn_ruminate);
   WAIT_STATE( ch, skill_table[gsn_ruminate]->beats );
   if( ch->max_mana > ch->mana + get_curr_int(ch) )
   {
      ch->mana = ch->mana + get_curr_int( ch ) + get_curr_wis( ch ) + ch->level;
   } else 
   {
      ch->mana = ch->max_mana;
   }
...
void do_visogy( CHAR_DATA * ch, char *argument )
{
...
void do_mistwalk( CHAR_DATA * ch, char *argument )
{
   ...
   {
      send_to_char( "You are not emo enough to mistwalk.\r\n", ch );
      return;
   }
...
void do_visible( CHAR_DATA * ch, char *argument )
{
   ...
   for( paf = ch->first_affect; paf; paf = paf_next )
   {
      ...
      if( xIS_SET( paf->bitvector, AFF_SNEAK )
          || xIS_SET( paf->bitvector, AFF_INVISIBLE ) || xIS_SET( paf->bitvector, AFF_HIDE ) )
      {
...
void do_recall( CHAR_DATA * ch, char *argument )
{
   ...
   location = NULL;
/*
   if( get_trust( ch ) >= 26 && get_trust( ch ) < 99 )
   {
      send_to_char( "Level's 26 through 99 cannot use the recall command.\r\n", ch );
      return;
   }
*/
   ...
   if( IS_NPC( ch ) )
   {
      if( IS_AFFECTED( ch, AFF_CHARM ) && ch->master && !IS_NPC( ch->master ) )
         do_emote( ch, "seems confused by their instructions." );
      else
         return;
   }
   else
   ...
   if( IS_SET( location->area->flags, AFLAG_CLOSED ) )
   {
      send_to_char( "That area is closed!\r\n", ch );
      return;
   }
...
void do_mount( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_mount ) )
   {
...
bool check_tumble( CHAR_DATA * ch, CHAR_DATA * victim )
{
   int chances;
/*
*   if( !is_class( victim, CLASS_THIEF ) || !IS_AWAKE( victim ) )
*      return FALSE;
*/
...
void do_poison_weapon( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_poison_weapon ) )
   {
...
/*    &&   ch->level < skill_table[gsn_scribe]->skill_level[ch->class] ) */
       && ch->level < SKILL_CLASS( ch, gsn_scribe ) )
   {
   ...
   if( scroll->value[1] == -1 )
   {
      ...
      act( AT_MAGIC, "You magically scribe a scroll.", ch, NULL, NULL, TO_CHAR );
...
void do_brew( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_brew ) )
   {
      send_to_char( "A skill such as this requires more magical ability than that of your class.\r\n", ch );
      return;
   }
...
void do_hitall( CHAR_DATA * ch, char *argument )
{
   ...
   for( vch = ch->in_room->first_person; vch; vch = vch_next )
	...
	if (vch==ch)
      ...
      act( AT_GREY, "Victim: $n ", ch, vch->name, NULL, TO_CHAR );
	 act( AT_GREY, "First: $n ", ch, ch->in_room->first_person->name, NULL, TO_CHAR );	
       act( AT_GREY, "Last: $n ", ch, ch->in_room->last_person->name, NULL, TO_CHAR );
return;	
   ...
   for( obj = ch->last_carrying; obj; obj = obj->prev_content )
   {
      if( can_see_obj( ch, obj ) )
      {
         if( obj->item_type == ITEM_QUIVER && !IS_SET( obj->value[1], CONT_CLOSED ) )
         {
            ...
            {
               if( obj2->item_type == ITEM_PROJECTILE)// && obj2->value[3] == type )//Commented by reaper
                  return obj2;
            }
         }
         if( obj->item_type == ITEM_PROJECTILE)// && obj->value[3] == type )//Commented by reaper
            return obj;
      }
   }
...
/* gsn_kick was used in this if statement , I fix - shogar */
   if( !IS_NPC( ch ) && !IS_IMMORTAL( ch )
        ...
        */
       && ch->level < SKILL_CLASS( ch, gsn_slice ) )
   {
...
/*  char buf[MAX_INPUT_LENGTH];
   ...
   if( !str_prefix( arg, "evasive" ) )
   {
      if(
            /*
             * ch->level < skill_table[gsn_style_evasive]->skill_level[ch->class]) 
             */
            ch->level < SKILL_CLASS( ch, gsn_style_evasive ) )
      {
   ...
   else if( !str_prefix( arg, "defensive" ) )
   {
      if(
            /*
             * ch->level < skill_table[gsn_style_defensive]->skill_level[ch->class]) 
             */
            ch->level < SKILL_CLASS( ch, gsn_style_defensive ) )
      {
   ...
   else if( !str_prefix( arg, "standard" ) )
   {
      if(
            /*
             * ch->level < skill_table[gsn_style_standard]->skill_level[ch->class])
             */
            ch->level < SKILL_CLASS( ch, gsn_style_standard ) )
      {
   ...
   else if( !str_prefix( arg, "aggressive" ) )
   {
      if(
            /*
             * ch->level < skill_table[gsn_style_aggressive]->skill_level[ch->class])
             */
            ch->level < SKILL_CLASS( ch, gsn_style_aggressive ) )
      {
   ...
   else if( !str_prefix( arg, "berserk" ) )
   {
      if(
            /*
             * ch->level < skill_table[gsn_style_berserk]->skill_level[ch->class])
             */
            ch->level < SKILL_CLASS( ch, gsn_style_berserk ) )
      {
...
void do_cook( CHAR_DATA * ch, char *argument )
{
   ...
   if( IS_NPC( ch ) ||
        ...
        */
       ch->level < SKILL_CLASS( ch, gsn_cook ) )
   {
