...
void medit_disp_npc_menu( DESCRIPTOR_DATA * d )
{
   ...
   damestimate += get_damroll( mob, TRUE );
   ...
   ch_printf_color( ch, "&gN&w) Gold:     [&c%8ld&w], &gO&w) Spec: &O%-22.22s\r\n", mob->gold, lookup_spec( mob->spec_fun ) );
...
void medit_disp_pc_menu( DESCRIPTOR_DATA * d )
{
   ...
   ch_printf_color( ch, "&gI&w) Gold:  [&c%11ld&w],  &gJ&w) Mentalstate:  [&c%5d&w],  &gK&w) Emotional: [&c%5d&w]\r\n",
