...
void do_reroll( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->level2 != -1 )
   {
      send_to_char( "Sorry, you can't reroll if your remorting!\r\n", ch );
      return;
   }
   if( ch->level3 != -1 )
   {
   ...
   sprintf( buf, "%-2d str %-2d int %-2d wis %-2d dex %-2d con %-2d cha %-2d lck.\r\n",
...
void do_gold( CHAR_DATA * ch, char *argument )
{
   ...
   if( IS_NPC( ch ) )
   {
      ch_printf( ch, "You have %s gold, ", num_punct( ch->pcdata->gbalance ) );
      ch_printf( ch, "%s silver", num_punct( ch->pcdata->sbalance ) );
      ch_printf( ch, ", and %s copper coins.\r\n", num_punct( ch->pcdata->balance ) );
   }
   else
      ch_printf( ch,
                 "You have %ld gold pieces, %ld silver pieces, %ld copper pieces on hand and %ld gold pieces, %ld silver pieces, %ld copper pieces in the bank.\r\n",
                 ch->gold, ch->silver, ch->copper, ch->pcdata->gbalance, ch->pcdata->sbalance, ch->pcdata->balance );
   return;
}
...
void do_worth( CHAR_DATA * ch, char *argument )
{
   ...
   pager_printf( ch, "|Glory: %-4d |Gold: %-11ld |Silver: %-12ld |Copper: %-13ld |\r\n",
                 ch->pcdata->quest_curr, ch->gold, ch->silver, ch->copper );
   pager_printf( ch, "|Bank        |Gold: %-11s", num_punct( ch->pcdata->gbalance ) );
   pager_printf( ch, " |Silver: %-13s|Copper: %-14s|\r\n",
                 num_punct( ch->pcdata->sbalance ), num_punct( ch->pcdata->balance ) );

   send_to_pager( " ----------------------------------------------------------------------------\r\n", ch );
   if( ch->level < 15 && !IS_PKILL( ch ) )
      pager_printf( ch, "|Weight: %-2d |Hitroll: -------- |Damroll: ----------- |Style: %-13s  |\r\n",
                    ch->carry_weight, buf );
   else
      pager_printf( ch, "|Weight: %-2d  |Hitroll: %-8d |Damroll: %-11d |Style: %-13s  |\r\n",
                    ch->carry_weight, get_hitroll( ch, FALSE ), get_damroll( ch, FALSE ), buf );
   send_to_pager( " ----------------------------------------------------------------------------\r\n", ch );
   return;
}
...
void do_score( CHAR_DATA * ch, char *argument )
{
   ...
   pager_printf( ch, "\r\n&CScore for: &W%s %s%s%s&C.\r\n",
                 ch->pcdata->extraname ? ch->pcdata->extraname : "",
                 ch->name, ch->pcdata->lastname ? ch->pcdata->lastname : "", ch->pcdata->title );

   if( get_trust( ch ) != ch->level && get_trust( ch ) != ch->level2 && get_trust( ch ) != ch->level3 )
      pager_printf( ch, "&CYou are trusted at level &W%d&C.\r\n", get_trust( ch ) );

   send_to_pager( "----------------------------------------------------------------------------\r\n", ch );

   pager_printf( ch, "&CLEVEL: &W%-3d         &CRace : &W%-10.10s        &CPlayed: &W%d &Chours\r\n",
                 ch->level, capitalize( get_race( ch ) ), ( get_age( ch ) - ( race_table[ch->race]->base_age + 17 ) ) * 2 );
   ...
   if( ch->level >= 15 || IS_PKILL( ch ) )
   {
      pager_printf( ch, "&CSTR  : &W%2.2d&C(&w%2.2d&C)    HitRoll: &R%-4d              &CSaved:  &W%s\r",
                    get_curr_str( ch ), ch->perm_str, get_hitroll( ch, FALSE ),
                    ch->save_time ? ctime( &( ch->save_time ) ) : "no save this session\n" );

      pager_printf( ch, "&CINT  : &W%2.2d&C(&w%2.2d&C)    DamRoll: &R%-4d              &CTime:   &W%s\r",
                    get_curr_int( ch ), ch->perm_int, get_damroll( ch, FALSE ), ctime( &current_time ) );
   }
   else
   {
   ...
   pager_printf( ch, "&COn Hand: &YGold: %-12ld  ", ch->gold );
   pager_printf( ch, "&WSilver: %-12ld ", ch->silver );
   pager_printf( ch, "&RCopper: %-12ld\r\n", ch->copper );

   pager_printf( ch, "&CBalance: &YGold: %-12ld &WSilver: %-12ld &RCopper: %-12ld\r\n", ch->pcdata->gbalance,
                 ch->pcdata->sbalance, ch->pcdata->balance );
   ...
   for( iLang = 0; lang_array[iLang] != LANG_UNKNOWN; iLang++ )
      if( knows_language( ch, lang_array[iLang] ) || ( IS_NPC( ch ) && ch->speaks == 0 ) )
      {
   ...
   if( ch->pcdata->arena_wins != 0 || ch->pcdata->arena_deaths != 0 || ch->pcdata->arena_kills != 0 )
   {
      send_to_pager( "&C----------------------------------------------------------------------------\r\n", ch );
      pager_printf( ch, "ARENA DATA:  Wins: (&W%3.3d&C)     Kills: (&W%3.3d&C)     Deaths: (&W%3.3d&C)\r\n",
                    ch->pcdata->arena_wins, ch->pcdata->arena_kills, ch->pcdata->arena_deaths );
   }
...
/*
        if( ch->pcdata->bamfin[0] == '\0' )
            pager_printf( ch, "&CBamfin : &YNot changed/Switched.\r\n", ch->name );
        else
            pager_printf( ch, "&CBamfin : &Y%s\r\n", ch->pcdata->bamfin );

        if( ch->pcdata->bamfout[0] == '\0' )
            pager_printf( ch, "&CBamfout: &YNot changed/Switched.\r\n", ch->name );
        else
            pager_printf( ch, "&CBamfout: &Y%s\r\n", ch->pcdata->bamfout );
*/
      pager_printf( ch, "&CBamfin : &Y%s\r\n",
                    ch->pcdata->bamfin[0] != '\0' ? ch->pcdata->bamfin : "Not changed/Switched." );
      pager_printf( ch, "&CBamfout: &Y%s\r\n",
                    ch->pcdata->bamfout[0] != '\0' ? ch->pcdata->bamfout : "Not changed/Switched." );
...
void do_newscore( CHAR_DATA * ch, char *argument )
{
   ...
   else if( ch->class == CLASS_WARRIOR && ch->level2 == -1 && ch->level3 == -1 )
...
char *in_comma( int number )
{
   char buf[200];
   static char buf_comma[200];
   int x_pcomma, ex_amount;
   size_t x;
   ...
   {
      if( ( x - ex_amount ) % 3 == 0 && x > 0 && ! ( x == 1 && ex_amount == 2 ) )
      {
         buf_comma[x_pcomma] = ',';
         x_pcomma++;
...
void do_newscore2( CHAR_DATA * ch, char *argument )
{
   ...
   sprintf( extrabuf, "%s ", ch->pcdata->extraname );
   sprintf( buf, "&WPlayer Information&w:\r\n" );
   sprintf( buf + strlen( buf ), "%s%s%s\r\n", ( ch->pcdata->extraname[0] != '\0' )
            ? extrabuf : "", ch->name, ch->pcdata->title );
   sprintf( buf + strlen( buf ), "&gLevel &W:&w &B%-4d         &gRace     &W:&w &B%-13s&gClass   &W:&w &B%s\r\n",
            ch->level, get_race( ch ), capitalize( get_class( ch ) ) );
   if( ch->level2 != -1 )
      sprintf( buf + strlen( buf ), "&gLevel2&W:&w &B%-4d                                 &gClass2  &W:&w &B%s\r\n",
               ch->level2, capitalize( get_class2( ch ) ) );
   if( ch->level3 != -1 )
      sprintf( buf + strlen( buf ), "&gLevel3&W:&w &B%-4d                                 &gClass3  &W:&w &B%s\r\n",
               ch->level3, capitalize( get_class3( ch ) ) );

   sprintf( buf + strlen( buf ), "&gAge   &W:&w &B%-4d         &gHitroll  &W:&w &B%-3d          &gDamroll &W:&w &B%d\r\n",
            get_age( ch ), get_hitroll( ch, FALSE ), get_damroll( ch, FALSE ) );
   sprintf( buf + strlen( buf ),
            "&gStr   &W:&w &B%-2d&W(&b%-2d&W)       &gInt      &W:&w &B%-2d&W(&b%-2d&W)       &gWis     &W:&w &B%-2d&W(&b%-2d&W)\r\n",
            get_curr_str( ch ), ch->perm_str, get_curr_int( ch ), ch->perm_int, get_curr_wis( ch ), ch->perm_wis );
   sprintf( buf + strlen( buf ),
            "&gDex   &W:&w &B%-2d&W(&b%-2d&W)       &gCon      &W:&w &B%-2d&W(&b%-2d&W)       &gCha     &W:&w &B%-2d&W(&b%-2d&W)\r\n",
            get_curr_dex( ch ), ch->perm_dex, get_curr_con( ch ), ch->perm_con, get_curr_cha( ch ), ch->perm_cha );
   sprintf( buf + strlen( buf ),
            "&gLuck  &W:&w &B%-2d&W(&b%-2d&W)       &gPractices&W:&w &B%-4d         &gPlayed  &W:&w &B%d&w &ghours&w\r\n\r\n",
            get_curr_lck( ch ), ch->perm_lck, ch->practice, ( ( ch->played + ( current_time - ch->logon ) ) / 3600 ) );

   sprintf( buf + strlen( buf ), "&gExperience&W:&w &B%-11s ", in_comma( ch->exp ) );
   sprintf( buf + strlen( buf ), "&W(&b%s TNL&W)&w\r\n", in_comma( exp_level( ch, ch->level + 1 ) - ch->exp ) );

   if( ch->alignment > 900 )
      strcpy( extrabuf, "devout" );
   else if( ch->alignment > 700 )
      strcpy( extrabuf, "noble" );
   else if( ch->alignment > 350 )
      strcpy( extrabuf, "honorable" );
   else if( ch->alignment > 100 )
      strcpy( extrabuf, "worthy" );
   else if( ch->alignment > -100 )
      strcpy( extrabuf, "neutral" );
   else if( ch->alignment > -350 )
      strcpy( extrabuf, "base" );
   else if( ch->alignment > -700 )
      strcpy( extrabuf, "evil" );
   else if( ch->alignment > -900 )
      strcpy( extrabuf, "ignoble" );
   else
      strcpy( extrabuf, "fiendish" );

   sprintf( buf + strlen( buf ), "&gAlign&W:&w &B%-4d &W(&G%-9s&W)    &gAC&W:&w &B%-4d         ",
            ch->alignment, extrabuf, GET_AC( ch ) );

   switch ( ch->position )
   {
      case POS_DEAD:
         strcpy( extrabuf, "Slowly decomposing" );
         break;
      case POS_MORTAL:
         strcpy( extrabuf, "Mortally wounded" );
         break;
      case POS_INCAP:
         strcpy( extrabuf, "Incapacitated" );
         break;
      case POS_STUNNED:
         strcpy( extrabuf, "Stunned" );
         break;
      case POS_SLEEPING:
         strcpy( extrabuf, "Sleeping" );
         break;
      case POS_RECOVER:
         strcpy( extrabuf, "Recovering" );
         break;
      case POS_RESTING:
         strcpy( extrabuf, "Resting" );
         break;
      case POS_STANDING:
         strcpy( extrabuf, "Standing" );
         break;
      case POS_FIGHTING:
         strcpy( extrabuf, "Fighting" );
         break;
      case POS_EVASIVE:
         strcpy( extrabuf, "Fighting (evasive)" ); /* Fighting style support -haus */
         break;
      case POS_DEFENSIVE:
         strcpy( extrabuf, "Fighting (defensive)" );
         break;
      case POS_AGGRESSIVE:
         strcpy( extrabuf, "Fighting (aggressive)" );
         break;
      case POS_BERSERK:
         strcpy( extrabuf, "Fighting (berserk)" );
         break;
      case POS_MOUNTED:
         strcpy( extrabuf, "Mounted" );
         break;
      case POS_SITTING:
         strcpy( extrabuf, "Sitting" );
         break;
      default:
         strcpy( extrabuf, "BUG! Contact Kratas!" );
         break;
   }

   sprintf( buf + strlen( buf ), "&gPosition&W:&w &B%s&w.\r\n", extrabuf );

   sprintf( buf + strlen( buf ), "&CGold: &Y%s ", num_punct( ch->gold ) );
   sprintf( buf + strlen( buf ), "&CSilver: &Y%s	", num_punct( ch->silver ) );
   sprintf( buf + strlen( buf ), "&CCopper: &Y%s\r\n", num_punct( ch->copper ) );
   sprintf( buf + strlen( buf ), "&Y%ld&w gold, &Y%ld&w silver, and &Y%ld&w copper in the bank.\r\n\r\n",
            ch->pcdata->gbalance, ch->pcdata->sbalance, ch->pcdata->balance );
   /*
    * sprintf( buf+strlen(buf), "&Y%s&w silver in the bank.\r\n\r\n", in_comma(ch->pcdata->sbalance) );
    * sprintf( buf+strlen(buf), "&Y%s&w copper in the bank.\r\n\r\n", in_comma(ch->pcdata->balance) );
    */

   sprintf( buf + strlen( buf ), "&rHitpoints&w: %-8d &rof&w %-8d\r\n", ch->hit, ch->max_hit );
   sprintf( buf + strlen( buf ), "&b     Mana&w: %-8d &bof&w %-8d\r\n", ch->mana, ch->max_mana );
   sprintf( buf + strlen( buf ), "&O     Move&w: %-8d &Oof&w %-8d\r\n\r\n", ch->move, ch->max_move );

   if( IS_IMMORTAL( ch ) )
   {
      if( ch->pcdata->area )
      {
         sprintf( buf + strlen( buf ), "&CImmortal Information&W:&w\t\t\t&CArea &c%s&C loaded&w.\r\n",
                  IS_SET( ch->pcdata->area->status, AREA_LOADED ) ? "is" : "is not" );
         sprintf( buf + strlen( buf ), "You &c%s&w WizInvis.\r\n", xIS_SET( ch->act, PLR_WIZINVIS ) ? "are" : "are not" );
         sprintf( buf + strlen( buf ), "Your current WizInvis level is &c%d&w.\r\n", ch->pcdata->wizinvis );
         if( ch->pcdata->bamfin[0] == '\0' )
            sprintf( buf + strlen( buf ), "&CBamfin &W:&w %s appears in a swirling mist.\r\n", ch->name );
         else
            sprintf( buf + strlen( buf ), "&CBamfin &W:&w %s\r\n", ch->pcdata->bamfin );
         if( ch->pcdata->bamfout[0] == '\0' )
            sprintf( buf + strlen( buf ), "&CBamfout&W:&w %s appears in a swirling mist.\r\n", ch->name );
         else
            sprintf( buf + strlen( buf ), "&CBamfout&W:&w %s\r\n", ch->pcdata->bamfout );

         sprintf( buf + strlen( buf ), "&CVnums for area&W:&w\r\n" );
         sprintf( buf + strlen( buf ),
                  "Rooms: (&C%-5.5d&w - &C%-5.5d&w)    Objects: (&C%-5.5d&w - &C%-5.5d&w)    Mobs: (&C%-5.5d&w - &C%-5.5d&w)\r\n",
                  ch->pcdata->area->low_r_vnum, ch->pcdata->area->hi_r_vnum, ch->pcdata->area->low_o_vnum,
                  ch->pcdata->area->hi_o_vnum, ch->pcdata->area->low_m_vnum, ch->pcdata->area->hi_m_vnum );
      }
      else
      {
         sprintf( buf + strlen( buf ), "&CImmortal Information&W:&w\r\n" );
         sprintf( buf + strlen( buf ), "You &c%s&w WizInvis.\r\n", xIS_SET( ch->act, PLR_WIZINVIS ) ? "are" : "are not" );
         sprintf( buf + strlen( buf ), "Your current WizInvis level is &c%d&w.\r\n", ch->pcdata->wizinvis );
   ...
   if( time_info.day == ch->day && time_info.month == ch->month )
      send_to_char( "\r\n &cToday is your birthday!\r\n", ch );
   else
      ch_printf( ch, "&cYour birthday is:\r\nDay of %s, %d%s day in the Month of %s, in the year %d.\r\n",
                 day_name[ch->day % 7], day, suf, month_name[ch->month], ch->year );
...
/* Returns Third class -- Josh */
char *get_class3( CHAR_DATA * ch )
{
   if( ch->class3 < MAX_NPC_CLASS && ch->class3 >= 0 )
      return ( npc_class[ch->class3] );
   return ( "Unknown" );
}
...
void do_oldscore( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( ch->position )
   {
      ...
      case POS_SLEEPING:
         ...
         break;
      case POS_RECOVER:
         send_to_pager( "You are recovering.\r\n", ch );
         break;
      case POS_RESTING:
   ...
   if( ch->level >= 15 || IS_PKILL( ch ) )
      pager_printf( ch, "Hitroll: %d  Damroll: %d.\r\n", get_hitroll( ch, FALSE ), get_damroll( ch, FALSE ) );
...
void do_level( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch, "\r\nExperience required, levels %d to %d:\r\n______________________________________________\r\n\r\n",
              lowlvl, hilvl );
   sprintf( buf, " exp  (You have: %14s)", in_comma( ch->exp ) );
   sprintf( buf2, " exp  (To level: %14s)", in_comma( exp_level( ch, ch->level + 1 ) - ch->exp ) );
...
void do_affected( CHAR_DATA * ch, char *argument )
{
   ...
   send_to_pager_color( "&z-=&w[ &WAffects Summary &w]&z=-&D\r\n", ch );
   ...
   if( ch->immune > 0 )
   {
      send_to_pager_color( "&R*&z You are immune to:&D ", ch );
      pager_printf_color( ch, "&C%s\r\n", flag_string( ch->immune, ris_flags ) );
   }
   else
   {
      send_to_pager_color( "&R*&z You are immune to:&c Nothing&D\r\n", ch );
   }

   if( ch->resistant > 0 )
   {
      send_to_pager_color( "&R*&z You are resistant to:&D ", ch );
      pager_printf_color( ch, "&C%s\r\n", flag_string( ch->resistant, ris_flags ) );
   }
   else
   {
      send_to_pager_color( "&R*&z You are resistant to:&c Nothing&D\r\n", ch );
   }

   if( ch->susceptible > 0 )
   {
      send_to_pager_color( "&R*&z You are susceptible to:&D ", ch );
      pager_printf_color( ch, "&C%s\r\n", flag_string( ch->susceptible, ris_flags ) );
   }
   else
   {
      send_to_pager_color( "&R*&z You are susceptible to:&c Nothing&D\r\n", ch );
   }

   /*
    * if ( ch->absorb > 0 )
    * {
    * send_to_pager_color( "&R* &zYou absorb:  ", ch );
    * pager_printf_color( ch, "&C%s\r\n", flag_string(ch->absorb, ris_flags) );
    * }
    * else
    * {
    * send_to_pager_color( "&R* &zYou absorb:  &cNothing&D\r\n", ch );
    * }
    */
   ...
   for( paf = ch->first_affect; paf; paf = paf->next )
      if( ( skill = get_skilltype( paf->type ) ) != NULL )
      {
         ...
         pager_printf( ch, "&W%d&z> &BSpell:&w %-18s &W- &PModifies&w %-18s&B by &Y%-4d&B for&R %-4d&B Seconds.&D\r\n", 
affnum,
   ...
   if( affnum == 0 )
   {
      send_to_pager_color( "&R* &wNo cantrip or skill affects you.\r\n\r\n", ch );
   }
   sprintf( buf, "\r\n&WTotal of &R%d&W affects.&D\r\n", affnum );
   send_to_pager( buf, ch );
   send_to_pager( "\r\n&z-=&w[ &WSaves vs Spells Resistances &w]&z=-&D\r\n\r\n", ch );
   sprintf( buf, "&R* &zSaves vs Spells: Spell Effects &W- &G%d\r\n", ch->saving_spell_staff );
   send_to_pager( buf, ch );
   sprintf( buf, "&R* &zSaves vs Spells: Paralysis     &W- &G%d\r\n", ch->saving_para_petri );
   send_to_pager( buf, ch );
   sprintf( buf, "&R* &zSaves vs Spells: Staves, Wands &W- &G%d\r\n", ch->saving_wand );
   send_to_pager( buf, ch );
   sprintf( buf, "&R* &zSaves vs Spells: Petrification &W- &G%d\r\n", ch->saving_poison_death );
   send_to_pager( buf, ch );
   sprintf( buf, "&R* &zSaves vs Spells: Breath Wpns   &W- &G%d\r\n&D", ch->saving_breath );
   send_to_pager( buf, ch );
   return;
}
...
/* Problem dividing by 0 very naughty - Garinan */
...
void do_homepage( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->level < 5 )
   {
      send_to_char( "Sorry... your level is too low to do that.\r\n", ch );
      return;
   }
...
CHAR_DATA *find_trainer( CHAR_DATA * ch )
{
   CHAR_DATA *tmp;

   for( tmp = ch->in_room->first_person; tmp; tmp = tmp->next_in_room )
      if( IS_NPC( tmp ) && xIS_SET( tmp->act, ACT_TRAIN ) )
         return tmp;
...
void do_train( CHAR_DATA * ch, char *argument )
{
   ...
   {
      char buf[MIL];
      send_to_char( "You can train the following stats:\r\n", ch );
      sprintf( buf, "%-15s%-15s%-15s%-15s%-15s\r\n"
               "%-15s%-15s%-15s%-15s%-15s\r\n",
               "strength", "intelligence", "wisdom", "dexterity", "constitution", "charisma", "luck", "hp", "mana", "move" );
      send_to_char( buf, ch );
      // Show what they can train
      return;
   }
   ...
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_STR ) ? 25 : 20;
      char buf[MIL];

      if( ch->practice - 10 < 0 )
      {
         sprintf( buf, "%s You don't have enough practices.", ch->name );
         do_say_to_char( trainer, buf );
         return;
      }

      if( ch->perm_str + 1 > max )
      {
         sprintf( buf, "%s You can't train that anymore!", ch->name );
         do_say_to_char( trainer, buf );
         return;
      }
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his strength.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "int" ) || !str_cmp( arg, "intelligence" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_INT ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his intelligence.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "wis" ) || !str_cmp( arg, "wisdom" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_WIS ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his wisdom.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "dex" ) || !str_cmp( arg, "dexterity" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_DEX ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his dexterity.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "con" ) || !str_cmp( arg, "constitution" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_CON ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his constitution.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "cha" ) || !str_cmp( arg, "charisma" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_CHA ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his charisma.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "lck" ) || !str_cmp( arg, "luck" ) )
   {
      int max = ( class_table[ch->class]->attr_prime == APPLY_LCK ) ? 25 : 20;
      char buf[MIL];
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his luck.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "hp" ) )
   {
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his hitpoints.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "mana" ) )
   {
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his mana.", ch, NULL, NULL, TO_ROOM );
      return;
   }
   else if( !str_cmp( arg, "move" ) )
   {
      ...
      sprintf( buf, "%s Enjoy!", ch->name );
      do_say_to_char( trainer, buf );
      act( AT_PLAIN, "$n trains his move.", ch, NULL, NULL, TO_ROOM );
      return;
   }
...
{
#ifdef TAX_PLAYERS
   long int gold = ch->gold;
   //  int mgold = (ch->level * 2000000);
   //  int mgold = (ch->level * 100000);
   long int mgold = 10000000;
   long int tax = ( ch->gold * .05 );
   ...
   {
      set_char_color( AT_WHITE, ch );
      ch_printf( ch, "You are level %d and carry more than %ld coins.\r\n", ch->level, mgold );
      ch_printf( ch, "You are being taxed 5%% percent (%ld coins) of your %ld coins,\r\n", tax, ch->gold );
      ch_printf( ch, "and that leaves you with %ld coins.\r\n\r\n", ( ch->gold - tax ) );
      ch->gold -= tax;
   }

   {
      long int banktax = ch->pcdata->balance * .05;
      long int balanceamount = ch->pcdata->balance;

      if( balanceamount > 500000 )
      {
         set_char_color( AT_WHITE, ch );
         ch_printf( ch, "You are carrying more than 500,000 coins in your bank account.\r\n"
                    "You're bank account is now being taxed 5%% to help reduce inflation\r\n"
                    "This reduces your bank account by %ld coins, leaving you with %ld coins.",
                    banktax, balanceamount - banktax );
         balanceamount -= banktax;
         ch->pcdata->balance = balanceamount;
      }
   }
   return;
#endif
...
void do_corpse( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' )
   {
      ...
      send_to_char( "  retrieve: Retrieves your corpse   1gp / level\r\n", ch );
      send_to_char( " Type corpse <type> for the service.\r\n", ch );
      return;
   }

   if( !str_cmp( arg, "retrieve" ) )
      cost = 100 * ch->level * ch->level;
   else
   {
   ...
   for( obj = first_object; obj; obj = obj->next )
   {
      if( !nifty_is_name( buf, obj->name ) )
         continue;

/* Fix for player having same name as obj -Garinan */
