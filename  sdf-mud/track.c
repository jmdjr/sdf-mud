...
void found_prey( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( !can_see( ch, victim ) )
   {
      ...
      switch ( number_bits( 2 ) )
      {
         ...
         case 1:
            ...
            break;
         case 2:
            sprintf( buf, "yell I'm going to tear %s apart!", victname );
            interpret( ch, buf );
            break;
         case 3:
   ...
   if( IS_SET( ch->in_room->room_flags, ROOM_SAFE ) )
   {
      ...
      switch ( number_bits( 2 ) )
      {
         case 0:
            ...
            sprintf( buf, "yell %s is a bloody coward!", victname );
            interpret( ch, buf );
            break;
         case 1:
            ...
            break;
         case 2:
            sprintf( buf, "yell %s is a yellow-bellied wimp!", victname );
            interpret( ch, buf );
            break;
         case 3:
   ...
   {
      case 0:
         sprintf( buf, "yell Your blood is mine, %s!", victname );
         interpret( ch, buf );
         break;
      case 1:
...
void do_tlist( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch ) && ch->pcdata->learned[gsn_track] <= 0 )
   {
      send_to_char( "You do not know the tracking skill yet.\r\n", ch );
      return;
   }
   ...
   for( target = first_char; target; target = target->next )
   {
      if( can_see( ch, target )
          && ( nifty_is_name( arg, target->name ) )
          && ( find_first_step( ch->in_room, target->in_room, maxdist ) != BFS_NO_PATH ) )
      {
         sprintf( buf, "%d.%s (Area: %s)\r\n", count, ( IS_NPC( target ) ) ? target->short_descr : target->name,
...
void do_hunt( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Who are you trying to hunt?\r\n", ch );
      return;
   }
   ...
   if( !IS_NPC( ch ) )
      maxdist = ( maxdist * LEARNED( ch, gsn_hunt ) ) / 100;
   ...
   switch ( dir )
   {
      ...
      default:
         ...
         {
            if( dir == DIR_NORTH )
               move_char( ch, get_exit( ch->in_room, DIR_NORTH ), FALSE );
            else if( dir == DIR_SOUTH )
               move_char( ch, get_exit( ch->in_room, DIR_SOUTH ), FALSE );
            else if( dir == DIR_EAST )
               move_char( ch, get_exit( ch->in_room, DIR_EAST ), FALSE );
            else if( dir == DIR_WEST )
               move_char( ch, get_exit( ch->in_room, DIR_WEST ), FALSE );
            else if( dir == DIR_UP )
               move_char( ch, get_exit( ch->in_room, DIR_UP ), FALSE );
            else if( dir == DIR_DOWN )
               move_char( ch, get_exit( ch->in_room, DIR_DOWN ), FALSE );
            else if( dir == DIR_NORTHEAST )
               move_char( ch, get_exit( ch->in_room, DIR_NORTHEAST ), FALSE );
            else if( dir == DIR_NORTHWEST )
               move_char( ch, get_exit( ch->in_room, DIR_NORTHWEST ), FALSE );
            else if( dir == DIR_SOUTHEAST )
               move_char( ch, get_exit( ch->in_room, DIR_SOUTHEAST ), FALSE );
            else if( dir == DIR_SOUTHWEST )
               move_char( ch, get_exit( ch->in_room, DIR_SOUTHWEST ), FALSE );
            else
