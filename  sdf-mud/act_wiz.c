...
char *const hair_colors[] = {
   "Black", "Brown", "White", "Gray", "Light Brown", "Dark Brown",
   "Blond", "Light Blond", "Dark Blond", "Golden Blond", "Brown Blond",
   "Red", "Red Brown", "Red Blond", "Blood Red", "Orange Red",
   "Silver", "Strawberry"
};
char *const hair_lengths[] = {
   "Bald", "Crew-Cut", "Short", "Medium", "Long", "Waist"
};
char *const eye_colors[] = {
   "Aqua", "Brown", "Green", "Teal", "Blue-Green", "Hazel", "Blue",
   "Hazel-Blue", "Hazel-Green", "Light Brown", "Baby Blue",
   "Jade", "Red", "Blood-Red", "Silver"
};
char *const skin_tones[] = {
   "Brown", "Pale", "Red", "White", "Yellow", "Tan", "Green", "Blue",
   "Gold", "Silver", "Bronze", "Copper", "Gray"
};
char *const builds[] = {
   "Bone", "Thin", "Medium", "Large", "Buff", "Hardy"
};
char *const heights[] = {
   "Midget", "Short", "Medium", "Tall", "Towering"
};
...
/* clans.c */
bool rem_clan_member( CLAN_DATA * clan, char *name );
bool rem_council_member( COUNCIL_DATA * council, char *name );
...
void do_wizhelp( CHAR_DATA * ch, char *argument )
{
   ...
   set_pager_color( AT_PLAIN, ch );
   /*
    * Only should go up to characters level
    * * for(level = LEVEL_IMMORTAL - 1; level <= LEVEL_SUPREME + 1; level++ )
    */
   for( level = LEVEL_IMMORTAL - 1; level <= get_trust( ch ); level++ )
   {
      pager_printf( ch, "&g[&WLevel %d&g]&c\r\n", level );
      for( hash = 0; hash < 126; hash++ )
         for( cmd = command_hash[hash]; cmd; cmd = cmd->next )
            if( cmd->level >= LEVEL_HERO && cmd->level <= get_trust( ch ) && cmd->cshow == 1 && cmd->level == level )
            {
               pager_printf( ch, "%-12s", cmd->name );
               if( ++col % 6 == 0 )
                  send_to_pager( "\r\n", ch );
...
void do_authorize( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg2[0] == '\0' || !str_cmp( arg2, "accept" ) || !str_cmp( arg2, "yes" ) )
   {
      ...
      to_channel( buf, "Auth", LEVEL_NEOPHYTE );
...
void do_bamfin( CHAR_DATA * ch, char *argument )
{
   ...
   {
      /*
       * Removed so we wouldn't have to deal with comparing strings in other files
...
void do_rank( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->level2 != -1 )
   {
      ...
      {
         send_to_char( "You need to be a demigod.\r\n", ch );
         return;
      }
   }
...
/*
 * Removes _all_ affects from a player.  Like a forced
 * 'dispel magic' or something.  Makes things easier.
 * -- digifuzz.
 */
void do_cleanse( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' || !arg )
   {
      send_to_char( "Bestow thy holy cleansing upon whom?\r\n", ch );
      return;
   }
   ...
   {
      sprintf( buf, "You cleanse their body, returning it to its original state.\r\n" );
      send_to_char( buf, ch );

      sprintf( buf, "%s's spirit flows through you, and you are cleansed.\r\n", PERS( victim, ch ) );
   ...
   {
      send_to_char( "It appears their body is already pure.\r\n", ch );
      return;
   }
...
void do_rat( CHAR_DATA * ch, char *argument )
{
   ...
   if( strncmp( argument, "rat", 3) == 0 )  /* we can't have a rant inside of a rat this is bad */
   {
      send_to_char( "rat inside of a rat OH MY!\r\n", ch );
      return;
   }
...
void do_rstat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf_color( ch, "&cVnum: &w%d   &cSector: &w%d (%s)   &cLight: &w%d",
                    location->vnum, location->sector_type, sector_translation[location->sector_type][1], location->light );
...
void do_ostat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf_color( ch, "&cCost:  &Y%ld &cGold,  &Y%ld &cSilver,  &Y%ld &cCopper\r\n",
...
/* lets see if we can make life easier , show the progtypes - shogar */
   ...
   /*
    * Make life even easier... Number the affects so they are easier to remove.
    * -- Kratas 
    */
   for( count = 1, paf = obj->first_affect; paf; paf = paf->next, count++ )
      ch_printf_color( ch, "&c(%2d)Affects &w%s &cby &w%d. (extra)\r\n",
                       count, affect_loc_name( paf->location ), paf->modifier );
   for( paf = obj->pIndexData->first_affect; paf; paf = paf->next, count++ )
      ch_printf_color( ch, "&c(%d)Affects &w%s &cby &w%d.\r\n", count, affect_loc_name( paf->location ), paf->modifier );
   return;
}
...
void do_mstat( CHAR_DATA * ch, char *argument )
{
   ...
   if( victim->class < MAX_NPC_CLASS && ( victim->class >= 0 || victim->class2 >= 0 || victim->class3 >= 0 )
       && victim->race < MAX_NPC_RACE && victim->race >= 0 )
   {
      pager_printf_color( ch, "&cLevel   : &P%-3d             &cClass  : &w%-2.2d/%-10s   &cRace      : &w%-2.2d/%-10s\r\n", victim->level, victim->class, npc_class[victim->class], victim->race, npc_race[victim->race]  /*,
                                                                                                                                                                                                                            * victim->position */  );
      pager_printf_color( ch, "&cLevel   : &P%-3d             &cClass  : &w%-2.2d/%-10s\r\n",
                          victim->level2, victim->class2, npc_class[victim->class2] );
      pager_printf_color( ch, "&cLevel   : &P%-3d             &cClass  : &w%-2.2d/%-10s\r\n",
                          victim->level3, victim->class3, npc_class[victim->class3] );
   ...
   pager_printf_color( ch, "&cHitroll : &C%-5d           &cAlign  : &w%-5d           &cArmorClass: &w%d\r\n",
                       get_hitroll( victim, TRUE ), victim->alignment, GET_AC( victim ) );
   pager_printf_color( ch, "&cDamroll : &C%-5d           &cWimpy  : &w%-5d           &cPosition  : &w%d\r\n",
                       get_damroll( victim, TRUE ), victim->wimpy, victim->position );
   ...
   if( !IS_NPC( victim ) )
      pager_printf_color( ch, "&cThirst: &w%d   &cFull: &w%d   &cDrunk: &w%d   &cBl: %d\r\n",
                          ...
                          victim->pcdata->condition[COND_DRUNK], victim->pcdata->condition[COND_BLEEDING] );
   else
      send_to_pager( "\r\n", ch );
   ...
   pager_printf_color( ch, "&cYear: &w%-5d  &cSecs: &w%d  &cTimer: &w%d  &cBalance: &w%ld\r\n",
                       get_age( victim ), ( int )victim->played, victim->timer,
                       IS_NPC( victim ) ? 0 : victim->pcdata->balance );
   pager_printf_color( ch, "&cGold: &Y%ld  &cSilver: &Y%ld  &cCopper: &Y%ld\r\n",
   ...
   for( x = 0; lang_array[x] != LANG_UNKNOWN; x++ )
      if( knows_language( victim, lang_array[x] ) || ( IS_NPC( victim ) && victim->speaks == 0 ) )
      {
   ...
   if( !IS_NPC( victim ) && victim->pcdata->arena_wins != 0 )
      pager_printf_color( ch, "&cArena Wins: &w%d\r\n", victim->pcdata->arena_wins );
...
void do_oldreboot( CHAR_DATA * ch, char *argument )
{
   char buf[MAX_STRING_LENGTH];
   CHAR_DATA *vch;
...
{
   char arg[MAX_INPUT_LENGTH];
   char buf[MAX_STRING_LENGTH], lockerpath[MAX_STRING_LENGTH];
   char buf2[MAX_STRING_LENGTH];
   char *name;
   CHAR_DATA *victim;
   AREA_DATA *pArea;
   ...
   locker = load_locker(victim);
   //locker = fwrite_locker(victim);
   if(locker)
   {
      sprintf( lockerpath, "%s%s", LOCKER_DIR, capitalize( victim->name ) );
      remove(lockerpath);
   }
...
void do_immortalize( CHAR_DATA * ch, char *argument )
{
   ...
   if( victim->pcdata->clan )
   {
      ...
      --victim->pcdata->clan->num_members;
...
void do_users2( CHAR_DATA * ch, char *argument )
{
   char buf[MAX_STRING_LENGTH];
   DESCRIPTOR_DATA *d;
   int count;
...
void do_users( CHAR_DATA * ch, char *argument )
{
   ...
   DESCRIPTOR_DATA *d;
   char arg[MAX_INPUT_LENGTH], *connected;
   int count;
   ...
   bprintf( buf, "\r\n&CDesc&W|&cConnected State &W|&GIdle &W|&YPlayer     &O@Host           &W|&rUsername\r\n" );
   bprintf( buf, "&W----+----------------+-----+---------------------------+-------------\r\n" );
   ...
   for( d = first_descriptor; d; d = d->next )
   {
      switch ( d->connected )
      {
         ...
         case CON_GET_OLD_PASSWORD:
            connected = "Get Old Password";
            break;
         case CON_CONFIRM_NEW_NAME:
            connected = "  Confirm Name  ";
            break;
         case CON_GET_NEW_PASSWORD:
            connected = "Get New Password";
            break;
         case CON_CONFIRM_NEW_PASSWORD:
            connected = "Confirm Password";
            break;
         case CON_GET_NEW_SEX:
            connected = "  Choosing sex  ";
            break;
         case CON_GET_NEW_CLASS:
            connected = " Choosing Class ";
            break;
         case CON_READ_MOTD:
            connected = "  Reading MOTD  ";
            break;
         case CON_GET_NEW_RACE:
            connected = "  Choosing Race ";
            break;
         case CON_GET_EMULATION:
            connected = "  Get Emulation ";
            break;
         case CON_GET_WANT_RIPANSI:
            connected = "  Get RIP/ANSI  ";
            break;
         case CON_TITLE:
            connected = "Looking at title";
            break;
         case CON_PRESS_ENTER:
            connected = " Awaiting ENTER ";
            break;
         case CON_WAIT_1:
            connected = "   CON_WAIT_1   ";
            break;
         case CON_WAIT_2:
            connected = "   CON_WAIT_2   ";
            break;
         case CON_WAIT_3:
            connected = "   CON_WAIT_3   ";
            break;
         case CON_ACCEPTED:
            connected = "    Accepted    ";
            break;
         case CON_GET_PKILL:
            ...
            break;
         case CON_READ_IMOTD:
            connected = "  Reading IMOTD ";
            break;
         case CON_COPYOVER_RECOVER:
            connected = "Copyover Recover";
            break;
         case CON_PLAYING:
            connected = "     Playing    ";
            break;
         case CON_EDITING:
            ...
            break;
         case CON_OEDIT:
            connected = " Object Editing ";
            break;
         case CON_MEDIT:
            connected = "   Mob Editing  ";
            break;
         case CON_REDIT:
            connected = "  Room Editing  ";
            break;
         case CON_MAINMENU:
            connected = "    Main Menu   ";
            break;
         case CON_MAINMENU2:
            connected = "   Main Menu2   ";
            break;
         case CON_ANSI:
            connected = " Get Ansi Option";
            break;
         case CON_GET_HAIR_COLOR:
            connected = " Get Hair Color ";
            break;
         case CON_GET_HAIR_LENGTH:
            connected = " Get Hair Length";
            break;
         case CON_GET_EYE_COLOR:
            connected = " Get Eye Color  ";
            break;
         case CON_GET_SKIN_TONE:
            connected = " Get Skin Tone  ";
            break;
         case CON_GET_BUILD:
            connected = "Get Player Build";
            break;
         case CON_GET_HEIGHT:
            connected = "   Get Height   ";
            break;
         case CON_NOTE_TO:
            connected = " Writing a note ";
            break;
      ...
      if( arg[0] == '\0' )
      {
         ...
         {
            count++;
            bprintf( buf, "&C %3d&W|&c%-15s&W|&G%5d&W|&Y%-11s&O@%-15s&W|&r%s\r\n",
                     d->descriptor, connected, d->idle / 4,
                     d->original ? d->original->name : d->character ? d->character->name : "none", d->host, d->user );
         }
      }
      else
      {
         ...
         {
            count++;
            bprintf( buf, "&C %3d&W|&c%15s&W|&G%5d&W|&Y%-12s&O@%-16s&W|&r%s\r\n",
                     d->descriptor, connected, d->idle / 4,
                     d->original ? d->original->name : d->character ? d->character->name : "none", d->host, d->user );
         }
      }
   }
   send_to_pager_color( buf->data, ch );
   pager_printf_color( ch, "&c%d user%s&w.\r\n", count, count == 1 ? "" : "s" );
   buffer_free( buf );
}
...
void do_mortalize( CHAR_DATA * ch, char *argument )
{
   ...
   {
      send_to_char( "&C&wNo player exists by that name.\r\n", ch );
      return;
   }
...
void do_loadup( CHAR_DATA * ch, char *argument )
{
   ...
   name[0] = UPPER( name[0] );
/* sprintf( fname, "%s%c/%s", PLAYER_DIR, tolower( name[0] ), capitalize( name ) );
 * This seems wrong, but it does fix the problem. Partially at least. Still crashes
 * with an unused name.
*/
   sprintf( fname, "%s%c/%s", "..player", tolower( name[0] ), capitalize( name ));
   ...
   else
   {
      ...
      if(loaded == FALSE)
      {
/* Was crashing the MUD when a non existant player was loaded up
 * so this is how I fixed it    -Andy
 */
         send_to_char("Couldn't load pfile. Maybe it does not exist?", ch);
         return;
      }
       add_char( d->character );
      old_room_vnum = d->character->in_room->vnum;
...
/* Added the burlap sack to the newbieset.  The sack is part of newacad.are
   called New Darkhaven Academy - Brittany */

   {

      OBJ_INDEX_DATA *obj_ind = get_obj_index( 10465 );
      if( obj_ind != NULL )
      {
         obj = create_object( obj_ind, 1 );
         obj_to_char( obj, victim );
...
struct tm *update_time( struct tm *old_time )
{
   ...
   utime = mktime( old_time );
   return localtime( &utime );
}
...
void do_destroy( CHAR_DATA * ch, char *argument )
{
   ...
   if( !rename( buf, buf2 ) )
   {
      ...
      {
         ch->pcdata->council->num_members--;
         rem_council_member( ch->pcdata->council, ch->name );
      }
      ...
      if( ch->pcdata->clan )
      {
         ch->pcdata->clan->num_members--;
         rem_clan_member( ch->pcdata->clan, ch->name );
      }
...
void do_cset( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      pager_printf_color( ch, "  &wAlias Wait: &W%-7d pulses           ", sysdata.alias_wait );
   ...
   {
      STRFREE( sysdata.homepage );
      sysdata.homepage = STRALLOC( argument );
      send_to_char( "Ok.\r\n", ch );
      return;
   }
   ...
   {
      /*
       * using less then 1 means no one can connect feel free to raise the 10 to how high you want
       * to be able to set it though 
       */
      if( level < 1 || level > 10 )
      {
         send_to_char( "Can't set mplimit less then 1 or more then 10\r\n", ch );
         return;
      }
   ...
   {
      /*
       * using less then 1 means not even 1 pkiller can be connected 
       */
      if( level < 1 || level > 10 )
      {
         send_to_char( "Can't set mplimit less then 1 or more then 10\r\n", ch );
   ...
   else if( !str_cmp( arg, "doubleexp" ) )
   {
      if( sysdata.double_exp )
         do_info( ch, "Double EXP is now off.\r\n" );
      else
         do_info( ch, "Double EXP is now on.\r\n" );

      sysdata.double_exp = !sysdata.double_exp;
   }
   else if( !str_cmp( arg, "neverempty" ) )  /* Altrag */
   {
      if( sysdata.neverempty )
         do_info( ch, "Never Empty is now off.\r\n" );
      else
         do_info( ch, "Never Empty is now on.\r\n" );

      sysdata.neverempty = !sysdata.neverempty;
   }
   ...
   else if( !str_cmp( arg, "webserver" ) )
   {
      ...
      {
         send_to_char( "Webserver shutdown.\r\n", ch );
         sysdata.webtoggle = FALSE;
         shutdown_web(  );
      }
      else
      {
         send_to_char( "Starting WebServer....\r\n", ch );
         if( init_web( sysdata.webport ) )
         {
            send_to_char( "Webserver start request succeded.\r\n", ch );
            sysdata.webtoggle = TRUE;
         }
         else
         {
            send_to_char( "Webserver start request failed.\r\n", ch );
            sysdata.webtoggle = FALSE;
         }
      }
   }
#endif
   else if( !str_cmp( arg, "webcounter" ) )
   {
      if( sysdata.webcounter == TRUE )
      {
         send_to_char( "WebCounter off.\r\n", ch );
         sysdata.webcounter = FALSE;
      }
      else
      {
         send_to_char( "WebCounter on.\r\n", ch );
         sysdata.webcounter = TRUE;
      }
   }
   else
   {
...
void do_cedit( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "log" ) )
   {
      int n_log = atoi( argument );

      if( n_log < 0 || n_log > LOG_COMM )
      {
...
void do_setrace( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "name" ) )
   {
      ...
      snprintf( fbuf, 256, "%s%s.race", RACEDIR, race->race_name );
   ...
   if( !str_cmp( arg2, "attribute" ) )
   {
      ...
      {
         send_to_char( "Syntax:\r\n", ch );
         send_to_char( "  setrace <race> attribute <type>\r\n", ch );
         send_to_char( "  Where type is the following:\r\n", ch );
         send_to_char( "  - haircolor\t- hairlength\t- eyecolor\r\n", ch );
         send_to_char( "  - skintone\t- build\t\t- height\r\n", ch );
         return;
      }

      if( !str_cmp( arg3, "haircolor" ) )
      {
         size_t bitv;
         ...
         if( argument[0] == '\0' )
         {
            ...
            bprintf( buffer, "&cSyntax&W:&c\r\n  setrace <race> attribute <attrib> <type>\r\n" );
            bprintf( buffer, "  Types&W:&w\r\n" );
            for( bitv = 0; bitv < ( sizeof( hair_colors ) / sizeof( hair_colors[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( buffer, "\r\n" );
               bprintf( buffer, "\t%s", hair_colors[bitv] );
               if( strlen( hair_colors[bitv] ) < 8 )
                  bprintf( buffer, "\t" );
            }
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( hair_colors ) / sizeof( hair_colors[0] ) ); bitv++ )
               if( !str_cmp( arg3, hair_colors[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[HAIR_COLOR], 1 << bitv );
         }
         return;
      }

      if( !str_cmp( arg3, "hairlength" ) )
      {
         size_t bitv;

         if( argument[0] == '\0' )
         {
            BUFFER *sbuf = buffer_new( 100 );
            int num = 0;

            bprintf( sbuf, "&cSyntax&W:&c\r\n  setrace <race> attribute <attrib> <type>\r\n" );
            bprintf( sbuf, "  Types&W:&w\r\n" );

            for( bitv = 0; bitv < ( sizeof( hair_lengths ) / sizeof( hair_lengths[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( sbuf, "\r\n" );
               bprintf( sbuf, "\t%s", hair_lengths[bitv] );
               if( strlen( hair_lengths[bitv] ) < 8 )
                  bprintf( sbuf, "\t" );
            }

            send_to_char( sbuf->data, ch );
            buffer_free( sbuf );
            return;
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( hair_lengths ) / sizeof( hair_lengths[0] ) ); bitv++ )
               if( !str_cmp( arg3, hair_lengths[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[HAIR_LENGTH], 1 << bitv );
         }
         return;
      }

      if( !str_cmp( arg3, "eyecolor" ) )
      {
         size_t bitv;
         ...
         if( argument[0] == '\0' )
         {
            ...
            bprintf( sbuf, "&cSyntax&W:&c\r\n  setrace <race> attribute <attrib> <type>\r\n" );
            bprintf( sbuf, "  Types&W:&w\r\n" );
            for( bitv = 0; bitv < ( sizeof( eye_colors ) / sizeof( eye_colors[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( sbuf, "\r\n" );
               bprintf( sbuf, "\t%s", eye_colors[bitv] );
               if( strlen( eye_colors[bitv] ) < 8 )
                  bprintf( sbuf, "\t" );
            }
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( eye_colors ) / sizeof( eye_colors[0] ) ); bitv++ )
               if( !str_cmp( arg3, eye_colors[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[EYE_COLOR], 1 << bitv );
         }
         return;
      }


      if( !str_cmp( arg3, "skintone" ) )
      {
         size_t bitv;
         ...
         if( argument[0] == '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( skin_tones ) / sizeof( skin_tones[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( sbuf, "\r\n" );
               bprintf( sbuf, "\t%s", skin_tones[bitv] );
               if( strlen( skin_tones[bitv] ) < 8 )
                  bprintf( sbuf, "\t" );
            }
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( skin_tones ) / sizeof( skin_tones[0] ) ); bitv++ )
               if( !str_cmp( arg3, skin_tones[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[SKIN_TONE], 1 << bitv );
         }
         return;
      }
      ...
      if( !str_cmp( arg3, "build" ) )
      {
         ...
         if( argument[0] == '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( builds ) / sizeof( builds[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( sbuf, "\r\n" );
               bprintf( sbuf, "\t%s", builds[bitv] );
               if( strlen( builds[bitv] ) < 8 )
                  bprintf( sbuf, "\t" );
            }
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( builds ) / sizeof( builds[0] ) ); bitv++ )
               if( !str_cmp( arg3, builds[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[RACE_BUILD], 1 << bitv );
         }
         return;
      }
      ...
      if( !str_cmp( arg3, "height" ) )
      {
         ...
         if( argument[0] == '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( heights ) / sizeof( heights[0] ) ); bitv++, num++ )
            {
               if( num % 4 == 0 )
                  bprintf( sbuf, "\r\n" );
               bprintf( sbuf, "\t%s", heights[bitv] );
               if( strlen( heights[bitv] ) < 8 )
                  bprintf( sbuf, "\t" );
            }
         ...
         while( argument[0] != '\0' )
         {
            ...
            for( bitv = 0; bitv < ( sizeof( heights ) / sizeof( heights[0] ) ); bitv++ )
               if( !str_cmp( arg3, heights[bitv] ) )
               {
                  found = TRUE;
                  break;
               }
            ...
            TOGGLE_BIT( race->attribs[RACE_HEIGHT], 1 << bitv );
         }
         return;
      }
   }
...
void do_showrace( CHAR_DATA * ch, char *argument )
{
   ...
   send_to_char( "Race Attributes: (&GE&w means it is Enabled)\r\n", ch );
   send_to_char( "  &cHair Colors:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( hair_colors ) / sizeof( hair_colors[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[HAIR_COLOR], 1 << bitv ) ? "&GE" : " ", hair_colors[bitv] );
   }

   send_to_char( "\r\n  &cHair Lengths:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( hair_lengths ) / sizeof( hair_lengths[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[HAIR_LENGTH], 1 << bitv ) ? "&GE" : " ", hair_lengths[bitv] );
   }

   send_to_char( "\r\n  &cEye Colors:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( eye_colors ) / sizeof( eye_colors[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[EYE_COLOR], 1 << bitv ) ? "&GE" : " ", eye_colors[bitv] );
   }

   send_to_char( "\r\n  &cSkin Tones:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( skin_tones ) / sizeof( skin_tones[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[SKIN_TONE], 1 << bitv ) ? "&GE" : " ", skin_tones[bitv] );
   }

   send_to_char( "\r\n  &cRace Builds:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( builds ) / sizeof( builds[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[RACE_BUILD], 1 << bitv ) ? "&GE" : " ", builds[bitv] );
   }

   send_to_char( "\r\n  &cRace Heights:&w", ch );
   for( bitv = 0, num = 0; bitv < ( sizeof( heights ) / sizeof( heights[0] ) ); bitv++, num++ )
   {
      if( num % 4 == 0 )
         ch_printf( ch, "\r\n  -" );
      ch_printf( ch, "&w(%s&w)%-15s", IS_SET( race->attribs[RACE_HEIGHT], 1 << bitv ) ? "&GE" : " ", heights[bitv] );
   }
...
void do_qpstat( CHAR_DATA * ch, char *argument )
{
   ...
   if( get_trust( ch ) < get_trust( victim ) )
   {
      ch_printf( ch, "&Y%s is too high a level for you to do that.\r\n", victim->name );
      return;
   }
...
void do_project( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg, "more" ) || !str_cmp( arg, "mine" ) )
   {
      ...
      for( pproject = first_project; pproject; pproject = pproject->next )
      {
         ...
         if( MINE && ( !pproject->owner || str_cmp( ch->name, pproject->owner ) )
            ...
            continue;
         num_logs = 0;
         for( n_log = pproject->first_log; n_log; n_log = n_log->next )
            num_logs++;
   ...
   if( !str_cmp( arg1, "delete" ) )
   {
      ...
      n_log = pproject->last_log;
      while( n_log )
      {
         UNLINK( n_log, pproject->first_log, pproject->last_log, next, prev );
         tlog = n_log->prev;
         free_note( n_log );
         n_log = tlog;
      }
...
void do_ipcompare( CHAR_DATA * ch, char *argument )
{
   ...
   else if( !str_cmp( arg, "pkill" ) )
   {
      ...
      for( d = first_descriptor; d; d = d->next )
      {
         ...
         if( ( ( d->connected != CON_PLAYING
                 && d->connected != CON_EDITING )
               || ( d->connected <= CON_NOTE_TO
                    && d->connected >= CON_NOTE_FINISH ) ) || d->character == NULL || !can_see( ch, d->character ) )
            continue;
         for( hmm = first_ip; hmm; hmm = hmm->next )
         {
            if( !str_cmp( hmm->host, d->host ) &&
   ...
   for( d = first_descriptor; d; d = d->next )
   {
      if( !d->character || ( ( d->connected != CON_PLAYING
                               && d->connected != CON_EDITING )
                             || ( d->connected <= CON_NOTE_TO
                                  && d->connected >= CON_NOTE_FINISH ) ) || !can_see( ch, d->character ) )
         continue;
...
void do_nuisance( CHAR_DATA * ch, char *argument )
{
   ...
   char arg2[MAX_INPUT_LENGTH];
   struct tm *now_time;
   int ntime = 0, max_time = 0, power = 1;
   ...
   if( ntime )
   {
      add_timer( victim, TIMER_NUISANCE, ( 28800 * ntime ), NULL, 0 );
...
void do_search_flags( CHAR_DATA * ch, char *argument )
{
   AREA_DATA *area;
   ...
   if( !arg || arg[0] == '\0' )
   {
      send_to_char( "You must supply a room flag to search for.\r\n", ch );
      return;
   }
   else
      value = get_rflag( arg );
   ...
   if( value < 0 || value > 31 )
   {
      ch_printf( ch, "&cUnknown Flag&W:&w %s\r\n", arg );
      return;
   }

   for( area = first_asort; area; area = area->next )
   {
      for( vnum = area->low_r_vnum; vnum <= area->hi_r_vnum; vnum++ )
      {
         ...
         if( IS_SET( room->room_flags, 1 << value ) )
         {
            char tmpBuf[500];

            count++;
            sprintf( tmpBuf, "&cFound a room with the &W%s&c flag in room &W%d&w.\r\n",
                     flag_string( 1 << value, r_flags ), room->vnum );
            strcat( buf, tmpBuf );
         }
      }
   }
   ...
   if( count == 0 )
   {
      ch_printf( ch, "I didn't find any rooms with the %s flag.\r\n", flag_string( 1 << value, r_flags ) );
   }
   else
      send_to_pager( buf, ch );
}
...
void do_smite( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( victim = get_char_world( ch, arg ) ) == NULL )
   {
      ch_printf( ch, "%s was not found in the world.\r\n", arg );
      return;
   }

   if( victim == ch )
   {
      send_to_char( "How about you just die instead...\r\n", ch );
      raw_kill( victim, ch );
      return;
   }

   if( ( get_trust( victim ) > get_trust( ch ) ) || !str_cmp( victim->name, "Badastaz" ) )
   {
      send_to_char( "You don't have the power to smite them.\r\n", ch );
      return;
   }

   act( AT_IMMORT, "&YA bolt of &Wl&wi&Wg&wh&Wt&wn&Wi&wn&Wg &Ystrikes down from the heavens and hits $n!",
        victim, NULL, NULL, TO_ROOM );

   act( AT_IMMORT, "&YA bolt of &Wl&wi&Wg&wh&Wt&wn&Wi&wn&Wg &Ystrikes down from the heavens and hits you!",
        victim, NULL, NULL, TO_CHAR );

   victim->hit = victim->hit * .5;
   victim->mana = victim->mana * .5;
   victim->move = victim->move * .5;

   affect.type = gsn_stun;
   affect.location = APPLY_AC;
   affect.modifier = 20;
   affect.duration = 1;
   affect.bitvector = meb( AFF_PARALYSIS );
   affect_to_char( victim, &affect );
   update_pos( victim );

   if( !IS_NPC( victim ) )
   {
      victim->pcdata->smites++;
      adjust_hiscore( "smite", victim, victim->pcdata->smites );
   }

   ch->pcdata->smited++;
   adjust_hiscore( "smiter", ch, ch->pcdata->smited );

   ch_printf( ch, "You have smitten %s!\r\n", victim->name );
}

/* Checks links of a specified type */
void do_check_links( CHAR_DATA * ch, char *argument )
{
   char arg[MIL];

   if( str_cmp( ch->name, "Kratas" ) )
   {
      send_to_char( "Sorry, only Kratas can run this command. (To prevent abuse)\r\n", ch );
      send_to_char( "Please contact him if you need to check a linked list.\r\n", ch );
      return;
   }

   if( !argument || argument[0] == '\0' )
   {
      send_to_char( "You can currently check the following links.\r\n", ch );
      send_to_char( "\tcharacter\tdescriptor\thelp\r\n", ch );
      return;
   }
   argument = one_argument( argument, arg );


   if( !str_cmp( arg, "character" ) )
   {
      send_to_char( "Checking character list.\r\n", ch );
      CHECK_LINKS( first_char, last_char, next, prev, CHAR_DATA );
      send_to_char( "Done checking list.\r\n", ch );
      return;
   }
   else if( !str_cmp( arg, "descriptor" ) )
   {
      send_to_char( "Checking Descriptor List.\r\n", ch );
   ...
   {
      send_to_char( "Checking help list.\r\n", ch );
      CHECK_LINKS( first_help, last_help, next, prev, HELP_DATA );
      send_to_char( "Done checking list.\r\n", ch );
      return;
   }
   else
   {
      send_to_char( "Unknown list.\r\n", ch );
      return;
   }
}

void do_superbless( CHAR_DATA * ch, char *argument )
{
   if( str_cmp( argument, "" ) )
      do_info( ch, argument );
   else
      do_info( ch, "The gods feel you deserve a gift." );
   do_for( ch, "mortals cast iceshield #" );
   do_for( ch, "mortals cast fireshield #" );
   do_for( ch, "mortals cast armor #" );
   do_for( ch, "mortals cast shield #" );
   do_for( ch, "mortals cast slink #" );
   do_for( ch, "mortals cast haste #" );
   do_for( ch, "mortals cast \'kindred strength\' #" );
   do_for( ch, "mortals cast \'inner warmth\' #" );
/*    do_for( ch, "mortals cast indignation #" ); */
   do_for( ch, "mortals cast infravision #" );
   do_for( ch, "mortals cast \'aqua breath\' #" );
/*    do_for( ch, "mortals cast benediction #" ); */
   do_for( ch, "mortals cast \'elven beauty\' #" );
   do_for( ch, "mortals cast \'dragon wit\' #" );
   do_for( ch, "mortals cast \'trollish vigor\' #" );
   do_for( ch, "mortals cast \'aqua breath\' #" );
   do_for( ch, "mortals cast sagacity #" );
   do_for( ch, "mortals cast sanctuary #" );
   do_for( ch, "mortals cast resilience #" );
   do_for( ch, "mortals cast bless #" );
   do_for( ch, "mortals cast \'leprechaun fortune\' #" );
   do_for( ch, "mortals cast barkskin #" );
   do_for( ch, "mortals cast flight #" );
   do_for( ch, "mortals cast float #" );
   do_for( ch, "mortals cast forcefield #" );
   do_for( ch, "mortals cast shockshield #" );
   do_restore( ch, "all" );
   return;
}
...
void do_sslist( CHAR_DATA * ch, char *argument )
{
   ...
   for( level = 0; level < MAX_LEVEL; level++ )
   {
      ...
      for( sn = 0; sn < top_sn; sn++ )
      {
         if( skill_table[sn]->skill_level[victim->class] != level )
            continue;

         cur = victim->pcdata->learned[sn];
         max = skill_table[sn]->skill_adept[victim->class];

         if( !IS_IMMORTAL( victim ) && ( level > LEVEL_HERO || cur > max ) )
            color = AT_RED;
         else
            color = AT_BLUE;
...
/* Immortals can examine players' containers with following command -Garinan */

void do_imm_exam( CHAR_DATA * ch, char *argument )
{
   CHAR_DATA *victim;
   OBJ_DATA *obj;
   char arg1[MIL];
   ...
   if( arg1[0] == '\0' || arg2[0] == '\0' )
   {
      send_to_char( "Syntax: Immexam <player> <container>\r\n", ch );
      return;
   }

   if( ( victim = get_char_world( ch, arg1 ) ) == NULL )
   {
      send_to_char( "That player is not playing right now.\r\n", ch );
      return;
   }

   if( ( obj = get_obj_carry( victim, arg2 ) ) == NULL )
   {
      send_to_char( "That player doesn't have that object.\r\n", ch );
      return;
   }
...
/* Level gains*/
   ...
   send_to_char( buf, ch );
   ch->exp = exp_level( ch, ch->level ) * UMAX( 1, ch->level );
...
void do_fshow( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Syntax:  fshow <moblog | bugs | ideas | typos | sys_bugs | last_bugs | helps>\n\r", ch );
      return;
   }
   ...
   {
      show_file(ch, NOHELP_FILE);
      return;
   }
   if( !str_cmp( arg, "ideas" ) )
   {
      show_file( ch, IDEA_FILE );
      return;
   }
   if( !str_cmp( arg, "typos" ) )
   {
...
void do_fclear( CHAR_DATA * ch, char *argument )
{
   ...
   {
      fp = fopen( MOBLOG_FILE, "w" );
      fclose( fp );
      send_to_char( "Mob Log cleared.\r\n", ch );
      return;
   }
   if( !str_cmp( argument, "bugs" ) )
   {
      fp = fopen( PBUG_FILE, "w" );
      fclose( fp );
      send_to_char( "Bug Log cleared.\r\n", ch );
      return;
   }
   if( !str_cmp( argument, "ideas" ) )
   {
      fp = fopen( IDEA_FILE, "w" );
      fclose( fp );
      send_to_char( "Idea Log cleared.\r\n", ch );
      return;
   }
   if( !str_cmp( argument, "typos" ) )
   {
      fp = fopen( TYPO_FILE, "w" );
      fclose( fp );
      send_to_char( "Typo Log cleared.\r\n", ch );
      return;
   }
   if( !str_cmp( argument, "death" ) )
   {
      fp = fopen( DT_FILE, "w" );
      fclose( fp );
      send_to_char( "death log cleared.\r\n", ch );
      return;
   }
   if( !str_cmp( argument, "helps" ) );
   {
      fp = fopen( NOHELP_FILE, "w" );
      fclose( fp );
      send_to_char("No-help log cleared\r\n", ch);
      return;
   }
   send_to_char( "No such log to be cleared.\r\n", ch );
   send_to_char( "Syntax: fclear <moblog | bugs | ideas | typos | helps | death | helps>\r\n", ch );
   return;
}
...
void do_pledge( CHAR_DATA * ch, char *argument )
{
   ...
   if( victim->level < 10 )
   {
      send_to_char( "Players must attain level 10 to pledge.\r\n", ch );
      return;
   }

   act( AT_MAGIC, "$n disappears in a bright light that burns your eyes.", victim, NULL, ch, TO_NOTVICT );
   char_from_room( victim );
   send_to_char( "You have sent your pledgling on their way.\r\n", ch );

/* 25100 is my room vnum for start of pledge trial area */
   char_to_room( victim, get_room_index( 25750 ) );
   act( AT_MAGIC, "$n appears in a bright light that burns your eyes.", victim, NULL, ch, TO_NOTVICT );
   do_look( victim, "auto" );
   ch_printf( victim, "A STAFF member has decided to sponsor you towards becoming a STAFF member.\r\n"
              "Good luck to you.\r\n" );
   return;
}
...
void do_wizinduct( CHAR_DATA * ch, char *argument )
{
   ...
   if( !strcmp( arg1, "outcast" ) )
   {
      ...
      else
      {
         ...
         {
            send_to_char( "They are not a member of any clan.\r\n", ch );
            return;
         }
   ...
   else
   {
      ...
      else
      {
         ...
         if( victim->pcdata->clan )
         {
            send_to_char( "They are already a member of a clan/guild!\r\n", ch );
            return;
         }
         ...
         if( clan->clan_type == CLAN_GUILD )
         {
            ...
            for( sn = 0; sn < top_sn; sn++ )
            {
               ...
               // victim->pcdata->learned[sn] = GET_ADEPT( victim, sn );
                  victim->pcdata->learned[sn] = get_adept_remort( victim, sn );
...
bool check_area_conflict(AREA_DATA *carea, int val, int rom)
{
	switch(rom)
	{
	case 1:
	if((val>=carea->low_r_vnum && val<=carea->hi_r_vnum))
		return TRUE;
	case 2:
	if((val>=carea->low_o_vnum && val<=carea->hi_o_vnum))
		return TRUE;
	case 3:
	if((val>=carea->low_m_vnum && val<=carea->hi_m_vnum))
		return TRUE;
	default:
		return FALSE;
	}
...
bool check_area_conflicts( int lo, int hi )
{
   ...
   for( area = first_area; area; area = area->next )
      if( check_area_conflict( area, lo, 1 ) && check_area_conflict(area,lo,2) && check_area_conflict(area,lo,3) && check_area_conflict( area, hi, 1 ) && check_area_conflict(area,hi,2) && check_area_conflict(area,hi,3))
         return TRUE;
...
void do_free_vnums( CHAR_DATA * ch, char *argument )
{
   ...
   if( low_range < 1 || low_range > 100000 )
   {
      send_to_char( "Invalid argument for bottom of range.\r\n", ch );
      return;
   }

   if( high_range < 1 || high_range > 100000 )
   {
...
void do_cleararea( CHAR_DATA * ch, char *argument )
{
    ...
    */
   room = make_room( victim->pcdata->r_range_lo, tarea );
   if( !room )
   {
   ...
   room = make_room( victim->pcdata->r_range_hi, tarea );
   if( !room )
   {
...
{
   short slot;
   short sn, sn_count, last_slot_found;
...
void do_promote( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "level" ) )
   {
      ...
      send_to_char( "You suddenly feel very strange...\r\n\r\n", victim );
      set_char_color( AT_LBLUE, victim );
      sprintf( buf, "\r\n\r\nPlease congratulate our newest &CA&YZ &WStaff member %s!", victim->name );
      echo_to_all( AT_WHITE, buf, ECHOTAR_ALL );
/* Cannot tell a lie, got the idea for interpret configurations from DotdII source - 
Vladaar*/
      interpret( victim, "listen all" );
      ...
      interpret( victim, "title the New &CA&YZ&G Staff Member." );
      interpret( victim, "save" );
      make_wizlist(  );
      interpret( victim, "immtalk Greetings everybody!" );
      return;
   }
   return;
}
