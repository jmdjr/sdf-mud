...
int get_exp_worth( CHAR_DATA * ch )
{
   ...
   n_exp = ch->level * ch->level * ch->level * 5;
   n_exp += ch->max_hit;
   n_exp -= ( ch->armor - 50 ) * 2;
   n_exp += ( ch->barenumdie * ch->baresizedie + get_damroll( ch, FALSE ) ) * 50;
   n_exp += get_hitroll( ch, FALSE ) * ch->level * 10;
   ...
   if( IS_AFFECTED( ch, AFF_SHOCKSHIELD ) )
      n_exp += n_exp * 1.2;
   n_exp = URANGE( MIN_EXP_WORTH, n_exp, MAX_EXP_WORTH );
 ...
 */
short level_exp( CHAR_DATA * ch, int n_exp )
{
   int x, lastx, y, tmp;
...
short get_trust( CHAR_DATA * ch )
{
   ...
   if( ch->level2 != -1 && ch->level2 > ch->level )
      return ch->level2;

   if( ch->level3 != -1 && ch->level3 > ( ch->level && ch->level2 ) )
      return ch->level3;
...
void modify_skill( CHAR_DATA * ch, int sn, int mod, bool fAdd )
{
   if( !IS_NPC( ch ) )
   {
      ...
      { 
       ...
       ch->pcdata->learned[sn] = URANGE( 0, ch->pcdata->learned[sn] + mod, get_adept_remort( ch, sn ) );
//     if( (ch->pcdata->learned[sn] == 0) && (skill_table[sn]->skill_level[ch->class] == 0)
      }
   }
}
      ...
      case APPLY_BLOOD:
         if( !IS_NPC( ch ) )
            ch->pcdata->condition[COND_BLOODTHIRST] =
             ...
             ch->pcdata->condition[COND_BLOODTHIRST] + mod;
             /* Per Snaga, level +10 is not a good blood upper limit.  Changing
                for now.  Can always change it back!  --Mordred 10 October, 2009 */
         break;
...
void char_from_room( CHAR_DATA * ch )
{
   ...
   /*
    * Unload the area in ~20 minutes 
    */
   ch->in_room->area->ttl = 18;
...
void char_to_room( CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex )
{
    ...
    * REMOVE THIS KRATAS 
    */
//    CHECK_LINKS( pRoomIndex->first_person, pRoomIndex->last_person,
//                 next_in_room, prev_in_room, CHAR_DATA );
   ...
   reset_age = area->reset_frequency ? area->reset_frequency : 15;
   if( area->age >= reset_age && !IS_SET( area->flags, AFLAG_PLR_LOADED ) && !IS_NPC( ch ) )
   {
      int numplayers = area->nplayer;
      area->nplayer = 0;
      reset_area( area );
      area->nplayer = numplayers;
      SET_BIT( area->flags, AFLAG_PLR_LOADED );
   }
...
CHAR_DATA *get_char_world( CHAR_DATA * ch, char *argument )
{
   ...
   for( wch = ch->in_room->first_person; wch; wch = wch->next_in_room )
      if( can_see( ch, wch )
          && ( nifty_is_name( arg, PERS( wch, ch ) ) || ( IS_NPC( wch ) && vnum == wch->pIndexData->vnum ) ) )
      {
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
   ...
   if( !IS_AFFECTED( ch, AFF_TRUESIGHT ) )
   {
      ...
      if( room_is_dark( ch->in_room ) && !IS_AFFECTED( ch, AFF_INFRARED ) && get_trust( ch ) != 1 )
         return FALSE;

      if( IS_AFFECTED( victim, AFF_INVISIBLE ) && !IS_AFFECTED( ch, AFF_DETECT_INVIS ) && get_trust( ch ) != 1)
         return FALSE;
...
/* This is what the noastral flag is for. -Andy
...
OBJ_DATA *group_object( OBJ_DATA * obj1, OBJ_DATA * obj2 )
{
   ...
   if( obj1->pIndexData == obj2->pIndexData && !str_cmp( obj1->name, obj2->name ) && !str_cmp( obj1->short_descr, obj2->short_descr ) && !str_cmp( obj1->description, obj2->description ) && !str_cmp( obj1->action_desc, obj2->action_desc ) && obj1->item_type == obj2->item_type && xSAME_BITS( obj1->extra_flags, obj2->extra_flags ) && obj1->magic_flags == obj2->magic_flags && obj1->wear_flags == obj2->wear_flags && obj1->wear_loc == obj2->wear_loc && obj1->weight == obj2->weight && obj1->gold_cost == obj2->gold_cost && obj1->silver_cost == obj2->silver_cost && obj1->copper_cost == obj2->copper_cost && obj1->level == obj2->level && obj1->timer == obj2->timer && obj1->value[0] == obj2->value[0] && obj1->value[1] == obj2->value[1] && obj1->value[2] == obj2->value[2] && obj1->value[3] == obj2->value[3] && obj1->value[4] == obj2->value[4] && obj1->value[5] == obj2->value[5] && !obj1->first_extradesc && !obj2->first_extradesc && !obj1->first_affect && !obj2->first_affect && !obj1->first_content && !obj2->first_content && obj1->count + obj2->count > 0 ) /* prevent count overflow */
   {
 ...
 */
void boost_economy( AREA_DATA * tarea, long int gold )
{
 ...
 */
void lower_economy( AREA_DATA * tarea, long int gold )
{
   while( gold >= 1000000000 )
   {
      --tarea->high_economy;
 ...
 */
bool economy_has( AREA_DATA * tarea, long int gold )
{
...
short find_old_age( CHAR_DATA * ch )
{
   ...
   ch->day = ( number_range( 1, 30 ) - 1 );  /* Assign random day of birth */
   ch->month = ( number_range( 1, 12 ) - 1 );   /* Assign random month of birth */
   ch->year = time_info.year - age; /* Assign birth year based on calculations above */
   return age;
}
...
short calculate_age( CHAR_DATA * ch )
{
   ...
   num_days = ( time_info.month + 1 ) * 30;
   num_days += time_info.day;
   ch_days = ( ch->month + 1 ) * 30;
   ch_days += ch->day;
   age = time_info.year - ch->year;
