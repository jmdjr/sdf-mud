...
bool is_class( CHAR_DATA * ch, int class )
{
   ...
   {
      if( ch->class == class || ch->class2 == class )
         return TRUE;
   }
   else
   {
      if( ch->class == class || ch->class2 == class || ch->class3 == class )
         return TRUE;
   }
   return FALSE;
}
...
void get_obj( CHAR_DATA * ch, OBJ_DATA * obj, OBJ_DATA * container )
{
   CLAN_DATA *clan;
   int weight;
   long int amt; /* gold per-race multipliers */
...
/*	if (!char_died) save_char_obj(ch); */
   ...
   if( obj->item_type == ITEM_GOLD || obj->item_type == ITEM_COPPER || obj->item_type == ITEM_SILVER )
   {
      amt = obj->value[0];
/*
   ...
   else
   {
      ...
      switch ( container->item_type )
      {
         ...
         case ITEM_CORPSE_PC:
         {
             ...
             */
            sprintf( corpsename, "corpse %s", ch->name );
            if( IS_OBJ_STAT( container, ITEM_CLANCORPSE )
                && !IS_NPC( ch )
                && container->action_desc[0] != '\0'
                && str_cmp(corpsename, container->name)
                && str_cmp( container->action_desc, ch->name ) )
            {
               sprintf(buf, "You did not inflict the death blow upon this corpse\r\n debug %s %s\r\n", ch->name, corpsename);
               send_to_char( buf, ch );
               return;
            }
      ...
      if( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
      {
         ...
         if( container->item_type == ITEM_CORPSE_PC )
         {
            ...
            {
               sprintf( buf, "%s is looting %s's corpse!  %s(room %d)", ch->name,
                        capitalize( name ), xIS_SET( obj->extra_flags, ITEM_CLANOBJECT ) ?
                        "(ClanObject) " : "", ch->in_room->vnum );
               log_string( buf );
               sprintf( buf, "%s is looting %s's corpse at %s!", ch->name, capitalize( name ), ch->in_room->name );
               do_info( NULL, buf );
            }
         }
         /*
...
void do_give( CHAR_DATA * ch, char *argument )
{
   ...
   if( is_number( arg1 ) )
   {
      ...
      amount = atoi( arg1 );
      if( amount <= 0 || ( str_cmp( arg2, "gold" ) && str_cmp( arg2, "silver" ) && str_cmp( arg2, "copper" ) ) )
      {
...
obj_ret damage_obj( OBJ_DATA * obj )
{
   ...
   if( obj->item_type != ITEM_LIGHT && !IS_OBJ_STAT( obj, ITEM_NOSCRAP ) && !in_arena( ch ) )
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit )
{
   ...
   /*
    * We now will keep people below level 50 from using
    * * objects above level 50 -- Kratas
    */

   /*
    * Put back to characters needing to be exact level of object
    * *         -Garinan
    */

   if( ( ch->level2 == -1 || ch->level3 == -1 ) && obj->level > get_trust( ch ) )
   {
   ...
   /*
    * Note, ch->level HAS to be used as opposed to get_trust because ch->level
    * * holds the remorted level!
    */
   else if( ( ch->level2 != -1 || ch->level3 != -1 ) && obj->level > ch->level && !IS_IMMORTAL( ch ) )
   {
      sprintf( buf, "You need to be level %d or above to use this object.\r\n", obj->level );
      send_to_char( buf, ch );
      act( AT_ACTION, "$n tries to use $p, but is too inexperienced.", ch, obj, NULL, TO_ROOM );
      return;
   }
   ...
   if( !IS_IMMORTAL( ch )
       && ( ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && is_class( ch, CLASS_WARRIOR ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && is_class( ch, 8 ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && is_class( ch, CLASS_MAGE ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_THIEF ) && is_class( ch, CLASS_THIEF ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_VAMPIRE ) && ch->race == RACE_VAMPIRE )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_DRUID ) && is_class( ch, CLASS_DRUID ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_WARRIOR ) && is_class( ch, CLASS_RANGER ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_MAGE ) && is_class( ch, CLASS_AUGURER ) )
            || ( IS_OBJ_STAT( obj, ITEM_ANTI_CLERIC ) && is_class( ch, CLASS_CLERIC ) ) ) )
   {
...
bool sac_obj( CHAR_DATA * ch, OBJ_DATA * obj, int obj_count )
{
   if( IS_SET( obj->magic_flags, ITEM_PKDISARMED ) )
   {
      if( CAN_PKILL( ch ) && !get_timer( ch, TIMER_PKILLED ) )
...
//        if( obj->first_content )
      {
         ch_printf( ch, "The contents of %s spill out!\r\n", obj->short_descr );

         for( tmpObj = obj->first_content; tmpObj; tmpObj = tmpObj->next )
         {
            obj_from_obj( tmpObj );
   ...
   // Bug fix for sacrifice all
   // separate_obj( obj );
   split_obj( obj, obj_count );
   extract_obj( obj );
...
void do_sacrifice( CHAR_DATA * ch, char *argument )
{
   ...
   else
   {
      static char *god_name_table[] = {
         "Badastaz", "Garinan", "Ender", "Dangel", "Doom", "Josh", "Recca", "Zyrrin",
      };
   ...
   if( !str_cmp( arg, "all" ) )
   {
      ...
      for( obj = ch->in_room->first_content; obj; obj = obj_next )
      {
         ...
         amount = obj->count;
         if( sac_obj( ch, obj, amount ) )
            count += amount;
      }
      if( count > 0 )
      {
         sprintf( buf, "(%d) %s gives you one copper coin for your sacrifice.\r\n", count, name );
         send_to_char( buf, ch );
   ...
   else
   {
      ...
      separate_obj( obj );
      /*
       * Allow saccing of any item that is able to be picked up -- Kratas
...
void do_auction( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( time_info.hour > 19 || time_info.hour < 7 ) && auction->item == NULL && !IS_IMMORTAL( ch ) )
   {
      ...
      if( auction->item != NULL )
      {
         ...
         if( auction->bet > 0 )
         {
            if( gbid <= 0 && sbid <= 0 && cbid > 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld copper\r\n", cbid );
            else if( gbid > 0 && sbid <= 0 && cbid > 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld gold and %ld copper\r\n", gbid, cbid );
            else if( gbid > 0 && sbid > 0 && cbid > 0 )
               sprintf( buf, "\r\nCurrent bid is %ld gold, %ld silver, and %ld copper\r\n", gbid, sbid, cbid );
            else if( gbid <= 0 && sbid > 0 && cbid > 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld silver and %ld copper\r\n", sbid, cbid );

            else if( gbid > 0 && sbid <= 0 && cbid <= 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld gold\r\n", gbid );
            else if( gbid <= 0 && sbid > 0 && cbid <= 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld silver\r\n", sbid );
            else if( gbid > 0 && sbid > 0 && cbid <= 0 )
               sprintf( buf, "\r\nCurrent bid on this item is %ld gold and %ld silver\r\n", gbid, sbid );
            else
               sprintf( buf, "Error! report to Ddruid.\r\n" );
         }
         else
         ...
         send_to_char( buf, ch );
         sprintf( buf,
                  "It value is: %ld gold, %ld silver, and %ld copper\r\nLevel: %d\r\n",
...
/*	    ch_printf( ch, "Bid: %d\r\n",newbet);	*/
         ...
         if( gbid > 0 && sbid > 0 && cbid > 0 )
            sprintf( buf, "A bid of %ld gold, %ld silver, and %ld copper has been received on %s.\r\n", gbid, sbid, cbid,
                     auction->item->short_descr );
         else if( gbid > 0 && sbid > 0 && cbid <= 0 )
            sprintf( buf, "A bid of %ld gold and %ld silver has been received on %s.\r\n", gbid, sbid,
                     auction->item->short_descr );
         else if( gbid > 0 && sbid <= 0 && cbid <= 0 )
            sprintf( buf, "A bid of %ld gold has been received on %s.\r\n", gbid, auction->item->short_descr );
         else if( gbid <= 0 && sbid > 0 && cbid <= 0 )
            sprintf( buf, "A bid of %ld silver has been received on %s.\r\n", sbid, auction->item->short_descr );
         else if( gbid <= 0 && sbid <= 0 && cbid > 0 )
            sprintf( buf, "A bid of %ld copper has been received on %s.\r\n", cbid, auction->item->short_descr );
         else if( gbid > 0 && sbid <= 0 && cbid > 0 )
            sprintf( buf, "A bid of %ld gold and %ld copper has been received on %s.\r\n", gbid, cbid,
                     auction->item->short_descr );
         else if( gbid <= 0 && sbid > 0 && cbid > 0 )
            sprintf( buf, "A bid of %ld silver and %ld copper has been received on %s.\r\n", sbid, cbid,
                     auction->item->short_descr );
         else
            sprintf( buf, "Error! report to Ddruid\r\n" );
         talk_auction( buf );
...
*/
         ...
         case ITEM_SCROLL:
            ...
            if( auction->starting == 0 )
               ...
               sprintf( buf, "A new item is being auctioned: %s at %ld gold, %ld silver, and %ld copper.", obj->short_descr,
                        gbid, sbid, cbid );
            else if( gbid > 0 && sbid > 0 && cbid <= 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld gold and %ld silver.", obj->short_descr, gbid, sbid );
            else if( gbid > 0 && sbid <= 0 && cbid <= 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld gold.", obj->short_descr, gbid );
            else if( gbid <= 0 && sbid > 0 && cbid <= 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld silver.", obj->short_descr, sbid );
            else if( gbid <= 0 && sbid <= 0 && cbid > 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld copper.", obj->short_descr, cbid );
            else if( gbid > 0 && sbid <= 0 && cbid > 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld gold and %ld copper.", obj->short_descr, gbid, cbid );
            else if( gbid > 0 && sbid > 0 && cbid > 0 )
               sprintf( buf, "A new item is being auctioned: %s at %ld silver and %ld copper", obj->short_descr, sbid, cbid );
            else
               sprintf( buf, "Error! report to Ddruid.\r\n" );
            talk_auction( buf );
 ...
 * Code courtesy of Stu, from the mailing list. Allows player to destroy
...
int find_rand_room( int vnum )
{
   ...
   /*
    * You'll have to replace sysdata.maxvnum with a hard coded number if you use
    * Stock Smaug 1.4 - Zarius 
    */
   for( i = 300; i <= 66000; i++ )
   {
   ...
   for( i = 300; i <= 66000; i++ )
   {
      if( ( room = get_room_index( i ) ) != NULL && number_range( 1, count * 2 ) == 1 )
         break;
      room = NULL;
   }
...
void do_roload( CHAR_DATA * ch, char *argument )
{
   ...
   do_info( ch, "The gods have loaded a special item for you to find." );
   argument = one_argument( argument, arg1 );
