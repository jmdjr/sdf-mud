            ...
            if( ch->pcdata->condition[COND_BLEEDING] > 0 )
            {
               ...
               if( number_range( 1, 8 ) > 1 )
               {
                  if( ch->pcdata->condition[COND_BLEEDING] != 1
                      && ( ch->position == POS_SITTING || ch->position == POS_SLEEPING || 
                           ch->position == POS_RESTING || ch->position == POS_RECOVER) )
                  {
...
void reboot_check( time_t reset )
{
   ...
   if( ( current_time % 1800 ) == 0 )
   {
      ...
      sprintf( buf, "%.24s:  %dptn  %dpll  %dsc %dbr  %ld gold loot  %ld silver loot   %ld Copper loot",
               ctime( &current_time ),
               sysdata.upotion_val,
               sysdata.upill_val,
