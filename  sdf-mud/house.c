...
void do_house( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg, "set" ) )
   {
      ...
      if( !str_cmp( arg3, "addroom" ) )
      {
         ...
         {
            if( homedata->vnum[x] == atoi( arg3 ) )
               found = TRUE;
            if( homedata->vnum[x] == 0 )
               break;
         }

         if( x == MAX_HOUSE_ROOMS )
         {
...
void do_homebuy( CHAR_DATA * ch, char *argument )
{
   ...
   if( !is_number( arg ) )
   {
      ...
      if( !str_cmp( arg, "add" ) )
      {
         ...
         sprintf( buf, "%s has decided to sell %s %s.", ch->name, ch->sex == 1 ? "his" : ch->sex == 2 ? "her" :
                  "its", apartment ? "apartment" : "house" );
         do_info( NULL, buf );
         return;
      }
      ...
      for( i = 1; i < MAX_HOUSE_ROOMS; i++ )
      {
         ...
         if( ADDED_ROOM_HOUSING_AREA )
         {
            ...
            for( tarea = first_build; tarea; tarea = tarea->next )
            {
               if( tarea == pArea )
               {
                  sprintf( fname, "%s%s", BUILD_DIR, pArea->filename );
                  fold_area( pArea, fname, FALSE );
               }
            }
         }
#endif
...
void homebuy_update(  )
{
   ...
   for( tmphome = first_homebuy; tmphome; tmphome = tmphomenext )
   {
      ...
      if( tmphome->seller[0] == '\0' || ( seller = load_player( tmphome->seller ) ) == NULL )
      {
         ...
         {
            sprintf( buf, "Homebuy_update: Residence with vnum %d could not be removed from housing auction",
                     tmphome->vnum );
            bug( buf, 0 );
         }
      ...
      if( !remove_house( seller ) && !IS_IMMORTAL( seller ) )
      {
         ...
         sprintf( buf, "Homebuy_update: Residence could not be removed from seller successfully for vnum %d",
                  tmphome->vnum );
         bug( buf, 0 );
         logoff( bidder );
         logoff( seller );
      ...
      if( bidder->desc )
      {
         ...
         sprintf( buf, "%s has successfully bought a new %s.", bidder->name, tmphome->apartment ? "apartment" : "house" );
         do_info( NULL, buf );
      }
      else
