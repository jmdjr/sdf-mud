...
long int get_cost args( ( CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, bool fBuy ) );
long int get_repaircost args( ( CHAR_DATA * keeper, OBJ_DATA * obj ) );
#undef CD
...
long int get_cost( CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, bool fBuy )
{
   SHOP_DATA *pShop;
   long int cost;
   bool richcustomer;
   ...
   if( fBuy )
   {
      ...
      cost = ( long int )( get_value( obj->gold_cost, obj->silver_cost, obj->copper_cost )
      ...
      cost = ( long int )( cost * ( 80 + UMIN( ch->level, LEVEL_AVATAR ) ) ) / 100;
   ...
   else
   {
      ...
      for( itype = 0; itype < MAX_TRADE; itype++ )
      {
         ...
         {
            cost = ( long int )( get_value( obj->gold_cost, obj->silver_cost, obj->copper_cost )
                            * UMIN( ( pShop->profit_buy - 1 ), pShop->profit_sell + profitmod ) ) / 100;
   ...
   if( obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND )
      cost = ( long int )( cost * obj->value[2] / obj->value[1] );

/* Glory Shops - Garinan */
   if( xIS_SET( keeper->act, ACT_GLORYSHOP ) )
   {
      cost = ( cost / 10 );
      if( xIS_SET( keeper->act, ACT_GLORYSHOP ) )
      {
...
long int get_repaircost( CHAR_DATA * keeper, OBJ_DATA * obj )
{
   REPAIR_DATA *rShop;
   long int cost;
   int itype;
   ...
   for( itype = 0; itype < MAX_FIX; itype++ )
   {
      ...
      {
         cost = ( long int )( get_value( obj->gold_cost, obj->silver_cost, obj->copper_cost ) * rShop->profit_fix / 1000 );
         found = TRUE;
         break;
      }
   }
...
/* Glory Shops - Garinan */
      ...
      {
         act( AT_TELL, "$n tells you 'You don't have enough glory to buy $p.'", keeper, obj, ch, TO_VICT );
         ch->reply = keeper;
         return;
      }

/* Modified for Glory Shops - Garinan */
      if( !xIS_SET( keeper->act, ACT_GLORYSHOP ) && plrmoney < tvalue )
      {
         act( AT_TELL, "$n tells you 'You can't afford to buy $p.'", keeper, obj, ch, TO_VICT );
         ch->reply = keeper;
         return;
...
void do_list( CHAR_DATA * ch, char *argument )
{
   ...
   if( IS_SET( ch->in_room->room_flags, ROOM_PET_SHOP ) )
   {
      ...
      CHAR_DATA *pet;
      bool found;
      long int gcost, scost, ccost, tmpvalue;
...
/*      bool listall; */
      ...
      for( obj = keeper->first_carrying; obj; obj = obj->next_content )
      {
             ...
             && ( arg[0] == '\0' || nifty_is_name( arg, obj->name ) ) )
         {
            ...
            {
               found = TRUE;
               pager_printf( ch, "[%-4s %-13s %-4s] Item\r\n", "Lv", "Cost G/S/C",
                             xIS_SET( keeper->act, ACT_GLORYSHOP ) ? "Glory" : "Gold" );
            }

            if( obj->level <= upper )
            {
               pager_printf( ch, "%s%2ld%s\r\n", divleft, upper, divright );
               upper = -1;
            }
             ...
             * Modified for Glory Shops - Garinan 
             */
            pager_printf( ch, "&c[&C%-4d %5ldg/%3lds/%3ldc %-4s&c] &w%s.\r\n",
                          obj->level, gcost, scost, ccost, xIS_SET( keeper->act, ACT_GLORYSHOP ) ? "&GQP" : "&YGold",
...
/* Glory Shops - Garinan */
   if( xIS_SET( keeper->act, ACT_GLORYSHOP ) )
   {
      send_to_char( "This shopkeeper won't buy anything from you.\r\n", ch );
      return;
   }
   ...
   if( gcost > 0 && scost > 0 && ccost > 0 )
      sprintf( buf, "You sell $p for %ld gold, %ld silver, %ld copper coins.", gcost, scost, ccost );
   else if( gcost > 0 && scost > 0 && ccost <= 0 )
      sprintf( buf, "You sell $p for %ld gold, and %ld silver coins.", gcost, scost );
   else if( gcost > 0 && scost <= 0 && ccost <= 0 )
      sprintf( buf, "You sell $p for %ld gold coin%s.", gcost, gcost == 1 ? "" : "s" );
   else if( gcost <= 0 && scost > 0 && ccost <= 0 )
      sprintf( buf, "You sell $p for %ld silver coin%s.", scost, scost == 1 ? "" : "s" );
   else if( gcost <= 0 && scost <= 0 && ccost > 0 )
      sprintf( buf, "You sell $p for %ld copper coin%s.", ccost, ccost == 1 ? "" : "s" );
   else if( gcost <= 0 && scost > 0 && ccost > 0 )
      sprintf( buf, "You sell $p for %ld silver, and %ld copper coins.", scost, ccost );
   else if( gcost > 0 && scost <= 0 && ccost > 0 )
      sprintf( buf, "You sell $p for %ld gold, and %ld copper coins.", gcost, ccost );
   else
      sprintf( buf, "Error, in if check!" );
...
void do_value( CHAR_DATA * ch, char *argument )
{
   ...
   if( gcost > 0 && scost > 0 && ccost > 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld gold, %ld silver, and %ld copper coins for $p.'", gcost, scost, ccost );
   else if( gcost > 0 && scost > 0 && ccost <= 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld gold, and %ld silver coins for $p.'", gcost, scost );
   else if( gcost > 0 && scost <= 0 && ccost <= 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld gold coins for $p.'", gcost );
   else if( gcost <= 0 && scost > 0 && ccost <= 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld silver coins for $p.'", scost );
   else if( gcost <= 0 && scost <= 0 && ccost > 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld copper coins for $p.'", ccost );
   else if( gcost <= 0 && scost > 0 && ccost > 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld silver, and %ld copper coins for $p.'", scost, ccost );
   else if( gcost > 0 && scost <= 0 && ccost > 0 )
      sprintf( buf, "$n tells you 'I'll give you %ld gold, and %ld copper coins for $p.'", gcost, ccost );
   else
      sprintf( buf, "Error in ifcheck report to Ddruid" );
                     ...
                     char *arg, int maxgold, char *fixstr, char *fixstr2 )
{
   ...
   else if( ( cost = strcmp( "all", arg ) ? cost : 11 * cost / 10 ) > plrmoney )
   {
      if( gcost > 0 && scost > 0 && ccost > 0 )
         sprintf( buf,
                  "$N tells you, 'It will cost %ld gold, %ld silver, and %ld copper coins to %s %s...'", gcost,
                  scost, ccost, fixstr, obj->name );
      else if( gcost > 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld silver coins to %s %s...'",
                  gcost, scost, fixstr, obj->name );
      else if( gcost > 0 && scost <= 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold coin%s to %s %s...'",
                  gcost, gcost == 1 ? "" : "s", fixstr, obj->name );
      else if( gcost <= 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld silver coin%s to %s %s...'",
                  scost, scost == 1 ? "" : "s", fixstr, obj->name );
      else if( gcost <= 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld copper coin%s to %s %s...'",
                  ccost, ccost == 1 ? "" : "s", fixstr, obj->name );
      else if( gcost <= 0 && scost > 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld silver, and %ld copper coins to %s %s...'",
                  scost, ccost, fixstr, obj->name );
      else if( gcost > 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld copper coins to %s %s...'", gcost,
   ...
   else
   {
      ...
      if( gcost > 0 && scost > 0 && ccost > 0 )
         sprintf( buf, "$N charges you %ld gold, %ld silver, and %ld copper coins to %s $p.", gcost, scost, ccost, fixstr );
      else if( gcost > 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N charges you %ld gold, and %ld silver coins to %s $p.", gcost, scost, fixstr );
      else if( gcost > 0 && scost <= 0 && ccost <= 0 )
         sprintf( buf, "$N charges you %ld gold coin%s to %s $p.", gcost, gcost == 1 ? "" : "s", fixstr );
      else if( gcost <= 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N charges you %ld silver coin%s to %s $p.", scost, scost == 1 ? "" : "s", fixstr );
      else if( gcost <= 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N charges you %ld copper coin%s to %s $p.", ccost, ccost == 1 ? "" : "s", fixstr );
      else if( gcost <= 0 && scost > 0 && ccost > 0 )
         sprintf( buf, "$N charges you %ld silver, and %ld copper coins to %s $p.", scost, ccost, fixstr );
      else if( gcost > 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N charges you %ld gold, and %ld copper coins to %s $p.", gcost, ccost, fixstr );
      else
         sprintf( buf, "Error in ifcheck report to Ddruid" );
...
void appraise_all( CHAR_DATA * ch, CHAR_DATA * keeper, char *fixstr )
{
   OBJ_DATA *obj;
   char buf[MAX_STRING_LENGTH];
   long int cost = 0, total = 0;
   long int gcost = 0, scost = 0, ccost = 0, tmpvalue = 0;
   ...
   if( total > 0 )
   {
      ...
      if( gcost > 0 && scost > 0 && ccost > 0 )
         sprintf( buf,
                  "$N tells you, 'It will cost %ld gold, %ld silver, and %ld copper coins in total.'", gcost, scost, ccost );
      else if( gcost > 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld silver coins in total.'", gcost, scost );
      else if( gcost > 0 && scost <= 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold coin%s in total.'", gcost, gcost == 1 ? "" : "s" );
      else if( gcost <= 0 && scost > 0 && ccost <= 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld silver coin%s in total.'", scost, scost == 1 ? "" : "s" );
      else if( gcost <= 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld copper coin%s in total.'", ccost, ccost == 1 ? "" : "s" );
      else if( gcost <= 0 && scost > 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld silver, and %ld copper coins in total.'", scost, ccost );
      else if( gcost > 0 && scost <= 0 && ccost > 0 )
         sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld copper coins in total.'", gcost, ccost );
      act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
...
void do_appraise( CHAR_DATA * ch, char *argument )
{
   ...
   if( gcost > 0 && scost > 0 && ccost > 0 )
      ...
      sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld silver coins to %s %s.'", gcost, scost, fixstr, obj->name );
   else if( gcost > 0 && scost <= 0 && ccost <= 0 )
      sprintf( buf, "$N tells you, 'It will cost %ld gold coin%s to %s %s.'",
               gcost, gcost == 1 ? "" : "s", fixstr, obj->name );
   else if( gcost <= 0 && scost > 0 && ccost <= 0 )
      ...
      sprintf( buf, "$N tells you, 'It will cost %ld gold, and %ld copper coins to %s %s.'", gcost, ccost, fixstr, obj->name );
...
/* Glory Shops - Garinan */
   if( xIS_SET( mob->act, ACT_GLORYSHOP ) )
      ch_printf( ch, "&GThis is a Glory Shop.\r\n" );
   return;
}
