...
/* canpkill altered to make up for rm's lack of an is_pkill bit
   now checks for deadly flag - Garinan (surprised?) */

      if( !str_cmp( chck, "canpkill" ) )
      {
         return ( IS_SET( chkchar->pcdata->flags, PCFLAG_DEADLY ) ? TRUE : FALSE );
      }
      ...
      {
         int level = chkchar->level;
         /*
          * return mprog_veval(get_trust(chkchar), opr, atoi(rval), mob); 
          */
         /*
          * Due to remort, we return the persons trust, or ch->level, rather than
          * * ch->level OR ch->level2 
          */
         if( chkchar->trust > chkchar->level )
            level = chkchar->trust;

         return mprog_veval( level, opr, atoi( rval ), mob );
      }
...
void mprog_speech_trigger( char *txt, CHAR_DATA * actor )
{
   ...
   {
      if( IS_NPC( vmob ) && HAS_PROG( vmob->pIndexData, SPEECH_PROG ) && knows_language( vmob, actor->speaking ) )
      {
