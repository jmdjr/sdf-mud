...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
   ...
   fprintf( fp, "GBalance     %ld\n", ch->pcdata->gbalance );
   fprintf( fp, "SBalance     %ld\n", ch->pcdata->sbalance );
...
void do_last( CHAR_DATA * ch, char *argument )
{
   ...
   if((IS_IMMORTAL(ch) && isdigit( arg[0] )) || atoi( arg ) == -1 || !str_cmp( arg, "today" ) )   //View list instead of players
   {
      send_to_char
