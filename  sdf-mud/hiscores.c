...
/* Hiscore command */
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      for( table = first_table; table; table = table->next )
      {
         ...
         {
            num = 1;
            ch_printf( ch, "  &C%-20s&c: keyword: &C%-10s&c: ", table->desc, table->keyword );
            for( entry = table->first_entry; entry; entry = entry->next )
            {
               if( num == 1 )
                  if( entry )
                     ch_printf( ch, "&C%s %d&D", capitalize( entry->name ), entry->score );
               num++;
            }
            send_to_char( "\r\n", ch );
            some_table = TRUE;
         ...
         else
         {
            ...
            {
               num = 1;
               ch_printf( ch, " &C %s&c: in object &C%s&c:", table->desc, oindex->short_descr );
               for( entry = table->first_entry; entry; entry = entry->next )
               {
                  if( num == 1 )
                     if( entry )
